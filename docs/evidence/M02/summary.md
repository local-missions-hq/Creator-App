...
- Creator and Business Maestro flows cover the local prototype from mission discovery or creation through the payment terminal states. The YAML parses successfully, and `pnpm test:e2e:mobile` statically verifies two flows plus source-backed test-ID references. Maestro `2.9.0` then executed both flows on the pinned iPhone 17 Pro Simulator: Creator passed in 27.036 seconds and Business passed in 23.724 seconds. JUnit, command, driver, and terminal screenshot artifacts are retained in [`maestro/`](./maestro/) and summarized in [`maestro-runs.md`](./maestro-runs.md). The real runs repaired partially visible bottom-action taps and replaced brittle compound-text/state assertions with explicit semantic terminal-state IDs.
