...
At this checkpoint, the pending financial intents were not yet a ledger, payable, refund, transfer, or promise that money moved. The later `M03-immutable-ledger-007` checkpoint now converts them into balanced internal creator-payable or business-refund obligations. M12 must still execute and reconcile Stripe test-mode webhook-authoritative charges, transfers, refunds, and payouts. Content-license activation, admin web API wiring, notifications, legal retention, and physical-device dispute UX remain later milestones.
