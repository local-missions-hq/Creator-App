...
- Six Reach qualification scenarios passed against real PostgreSQL: disabled-by-default capabilities and Community isolation, mandatory activation review and Creator consent, exact approved read-only sources, independent per-platform 90-day tiers, exact-platform/tier reservation concurrency, derived-only Business visibility, consent revocation, one outage-specific 14-day grace, independent appeal, and evidence minimization.
- Community applications never read Reach consent, provider availability, audience size, or tier. Reach evidence retains private provider references and an estimated local-audience count only until the 30-day deletion job succeeds; the retained qualification contains platform, tier, source/methodology, and dates. Businesses never receive the raw count, private evidence, combined audience, or unrelated-platform analytics.
- The complete checkpoint passes 88 database tests plus twelve API integration tests. `pnpm verify` passes all nine workspaces; repeated seed and `db:check` verify 87 tables; `drizzle-kit check` and the manifest validate all seventeen migrations; the security scan passes 369 text files; and Gitleaks finds no leak in approximately 12.32 MB.
- Reach qualification JUnit and review: [`test-results/reach-qualification-junit.xml`](./test-results/reach-qualification-junit.xml) and [`reach-qualification.md`](./reach-qualification.md).
...
- Added authenticated Creator Reach status plus per-platform consent grant/revoke and Business fixed-package availability routes.
- Business receives only the 80% Community floor, fixed reward multipliers, and capability availability. Raw/combined audience counts, evidence/provider references, and follower filters are absent.
- Regenerated deterministic production/local OpenAPI and the shared client. Explicit mobile adapters preserve provider-free local preview and require authentication in API mode.
