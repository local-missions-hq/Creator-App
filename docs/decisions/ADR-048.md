# ADR-048: Same-day destroy removes all disposable Azure workload resources while a separately managed minimal control plane retains Terraform state, OIDC/identity registrations, domain/DNS ownership, budgets/policy, code, and sanitized evidence
...
Same-day destroy removes all disposable Azure workload resources while a separately managed minimal control plane retains Terraform state, OIDC/identity registrations, domain/DNS ownership, budgets/policy, code, and sanitized evidence
