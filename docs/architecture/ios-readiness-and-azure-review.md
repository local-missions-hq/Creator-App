...
Repository baseline reviewed: `d31d92a` on `main` / `origin/main`; current checkpoint: `M05-github-oidc-arm-proof-ready-023`
...
| Azure runtime                  | Retained control plane live     | Dedicated subscription, three-resource state bootstrap, Entra-backed remote state, and exact 20-resource retained control plane independently verified; the retained workload landing zone is empty | Prove GitHub OIDC access without apply, then separately review/approve the 27-resource core, images, three-app activation, cloud tests, and teardown                             |
...
The dedicated subscription, three-resource retained-state bootstrap, and exact 20-resource retained control plane are live. Remote locking, exact inventory, immutable federation, scoped RBAC, budget/alerts, and a zero-change normal Terraform plan passed. The current gate is a separately approved no-apply GitHub OIDC proof before removing temporary operator access. Verification established:
...
The bootstrap and control-plane apply authorizations have been consumed. They do not authorize GitHub workflow activation, temporary-role removal, registry push, application deployment, live tests, rollback, restore, or destroy. Every later external action remains separately reviewed and approved.
