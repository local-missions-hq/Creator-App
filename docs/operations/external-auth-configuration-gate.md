...
2. A production-buildable mobile token adapter now POSTs `application/x-www-form-urlencoded` only to the exact configured HTTPS token endpoint. Authorization-code requests include the code, mobile client ID, exact redirect, and PKCE verifier; refresh requests include only the refresh grant, mobile client ID, and protected refresh credential. The adapter sends no client secret, omits credentials/referrer data, rejects redirects, uses a ten-second timeout and 64 KiB response limit, requires JSON, and maps every provider/network/body failure to one generic result without logging request or response bodies. The installed app runtime still uses the unavailable adapter, so this implementation cannot make a request until a later reviewed activation.
3. A production-buildable mobile JWKS resolver now uses only the configured same-origin HTTPS JWKS URI, omits credentials/referrer data, rejects redirects and header-supplied key locations, limits JSON to 64 KiB and twenty keys, and uses a three-second timeout, ten-minute cache, thirty-second cooldown, and unknown-key rotation reload. Locally injected fixtures prove this path; no provider/JWKS endpoint has been contacted, and the installed app runtime does not instantiate the resolver.
...
- Activate only the approved nonproduction environment. Wiring the reviewed token/JWKS adapters into the installed runtime is part of that separate activation change, not this local transport checkpoint. Confirm absent/partial/wrong-host/wrong-scope settings still fail closed.
...
This local checkpoint claims only a reviewed, machine-validated handoff and locally fixture-backed transport implementation. Entra tenant/app/API registration, runtime activation, provider enablement, provider/JWKS network contact, real identities, production authentication, and physical-iPhone execution remain open M4 gates.
