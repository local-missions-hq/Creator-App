...
The repository transfer and both three-update applies are complete. The first failed proof gained no Azure access and exposed a narrow subject-format mismatch. Exact correction plan SHA-256 `de06a09c687092fce1af5476b9ff37fa82d41039c13130e7f51f6395a55f923c` applied with zero creates/deletes and corrected run `33521970773` passed all three OIDC/ARM jobs. The expected Blob refusal proves default deny remained closed; it is not end-to-end workflow state access. Live read-only reconciliation subsequently proved plan read-only, apply no-delete, destroy no landing-zone deletion, and five-role ServicePrincipal-only constrained delegation. Provider-backed Terraform remains on the reviewed local operator path and the temporary operator role remains retained. The next task is the local activation-valid V2 lifecycle contract, not workload planning or apply.
