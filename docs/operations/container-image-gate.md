...
The command builds all workspaces, creates temporary production-only API and worker bundles from the frozen lockfile, assembles the Next.js standalone dashboard bundle, and verifies:
...
Temporary bundles are removed after verification. The production install is frozen and offline. Legacy pnpm deployment uses `--prefer-offline`: it reuses the content-addressable store first but may fetch missing package metadata while preserving the lockfile's exact versions and integrities. The command does not pull a base image or contact a container registry.
