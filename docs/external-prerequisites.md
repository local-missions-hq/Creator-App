...
| `AZURE_EPHEMERAL_BUDGET_POLICY` | Approved `$5` per run, `$100` monthly alert budget, 50%/80%/100% actual and forecast alerts, same-day scoped destroy | Blake Tindol, Cost + Technical owner | Any provider-backed workload plan | Amount and destination supplied; delivery, subscription budget apply, and scoped cleanup identity remain separately gated                               |
