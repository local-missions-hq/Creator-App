# Local Missions — Historical Completion Log and Legacy Build Plan

> Archived from `plans.md` on 2026-08-31. This file preserves the former 4,500-line
> build contract, detailed specifications, verification protocols, and dated work
...
- [x] Complete checkpoint `M03-mission-contract-capacity-003`: four versioned mission templates, objective campaign briefs, Community/Reach slot economics, 80% Community enforcement, qualified creator applications, atomic reservations, duplicate/capacity race proof, withdrawal replacement, and tenant-scoped acceptance.
- [x] Complete checkpoint `M03-check-in-state-machine-004`: accepted-mission schedules, venue-scoped staff assignments, hashed rotating QR/staff-code challenges, privacy-minimized check-in events, server-time enforcement, replay protection, and negative authorization/venue/expiry proofs.
- [x] Complete checkpoint `M03-submission-review-005`: versioned objective deliverables, accepted-brief assignment binding, private media metadata and quarantine state, check-in-gated complete submissions, one bounded correction, tenant-scoped review, 48-hour server-time auto-approval, and atomic negative/concurrency proofs.
- [x] Complete checkpoint `M03-dispute-resolution-006`: objective business and creator disputes, mission-bound structured evidence, PostgreSQL deadlines, independent platform review, all-or-nothing outcomes, immutable financial-action intents, and atomic authorization/duplicate/approval/resolution race proofs.
- [x] Complete checkpoint `M03-immutable-ledger-007`: provider-authoritative campaign funding, immutable provider references and slot allocations, deterministic 15% fee rounding, database-enforced balanced append-only journals, full creator/refund obligation posting, Finance Operator separation, and atomic idempotency/concurrency proofs.
- [x] Complete checkpoint `M03-local-pass-008`: hashed opaque creator/campaign links, venue offer inventory, HMAC-style synthetic customer deduplication, first-valid-claim attribution, seven-day claims, rotating claim-token hashes, future-claim pause, venue-scoped single-use redemption, explicit evidence, immutable histories, and duplicate/inventory/replay/authorization concurrency proofs.
- [x] Complete checkpoint `M03-content-rights-009`: immutable legal-document versions and Creator contract acceptance, fixed 90-day organic/12-month owned-media/30-day paid-ad licenses, Community-safe contract add-on compensation, full-payout-only activation, exact accepted assets/channels, expiry history, tenant isolation, and no-rights/concurrency/immutability proofs.
- [x] Complete checkpoint `M03-notification-outbox-010`: fixed transactional event catalog, versioned preferences/history, same-transaction mission-acceptance fan-out, minimized immutable events, leased outbox/history, durable recipient-only inbox, idempotent attempts, opt-out, exponential retry, dead-letter/admin replay, and deployment-forbidden local no-send proofs.
- [x] Complete checkpoint `M03-api-foundation-011`: production/local module separation, `/v1` discovery and stable mission-template cursor pagination, standard bounded errors, request/correlation IDs, allowlisted structured logs, live/ready/build/OpenAPI endpoints, production-impossible synthetic dev tokens, deterministic reviewed OpenAPI snapshots, and one HTTPS-safe generated client shared by mobile and dashboard.
- [x] Complete checkpoint `M03-migration-recovery-012`: deterministic thirteen-migration manifest/hash enforcement, empty-database proof, N-1 retained-data upgrade, injected transactional failure rollback, idempotent forward backfill, and a production-safe forward-only recovery runbook.
- [x] Complete checkpoint `M03-locality-proof-retention-013`: private proof references, objective correction/review/independent appeal, annual expiry, immediate address-change invalidation, 30-day post-review-or-appeal deletion, bounded expiring legal holds, leased retries/dead-letter alerts, immutable histories, and Community-eligibility enforcement.
- [x] Complete checkpoint `M03-authenticated-domain-api-014`: fail-closed production bearer boundary, local-only synthetic verification, `/v1/me`, Creator mission feed/detail/application, Business campaign reads, role/tenant isolation, transactional application retries, stable pagination, reviewed OpenAPI/client regeneration, and explicit phone-free mobile preview adapters.
- [x] Complete checkpoint `M03-local-pass-claim-edge-015`: five-minute single-use customer challenges, bounded verification/resend/risk limits, fresh pass recovery, refusal and substitute-acceptance evidence, independent Trust and Safety review, active-pass preservation, customer-safe fulfillment status, aggregate-only Business/Creator reporting, retention deadlines, tenant isolation, and negative/concurrency proofs without provider delivery.
- [x] Complete checkpoint `M03-content-rights-renewal-016`: Creator-visible renewal requests only within 30 days of expiry, fixed +25%/+50%/+100% rewards from the original locked base reward, explicit Creator accept/refuse without reliability effect, separate Business funding intent, authoritative funding activation, full Creator-payable obligation, exact non-backdated terms, one-time expiry-reminder audit checkpoints, and archive/removal enforcement policy without automatic renewal or a live Stripe call.
- [x] Complete checkpoint `M03-reach-qualification-017`: optional consented Instagram/TikTok/YouTube qualification with independent 90-day local-audience tiers, disabled-by-default provider capabilities, approved read-only evidence only, exact-platform/tier Reach reservations, Community isolation, derived-only Business visibility, one outage-specific 14-day grace, independent appeal, 30-day evidence deletion, and negative/concurrency/migration-recovery proofs without contacting a social provider.
- [x] Complete checkpoint `M03-authenticated-reach-api-018`: authenticated Creator Reach status and consent/revocation routes, fixed Business Reach packages and capability availability, derived-only privacy contracts, role isolation, deterministic OpenAPI/client regeneration, explicit provider-free mobile adapters, and iPhone-sized Creator/Business visual proof while every provider remains disabled.
- [x] Complete checkpoint `M03-completion-audit-019`: privacy-safe venue contacts, explicit Local Pass attribution confidence, audit timeline indexing, latest-schema recovery repair, consolidated transition matrix, and final M3 evidence gate.
- [x] Create the transactional PostgreSQL schema, migrations, indexes, constraints, UTC timestamps, integer-minor-unit money fields, stable public IDs, and deterministic synthetic seed.
- [x] Define the `/v1` REST contract, standard errors, pagination, request IDs, structured logs, health/build endpoints, and a production-impossible local dev-token boundary.
- [x] Generate and intentionally review the OpenAPI artifact and typed mobile/dashboard client contract.
- [x] Implement domain state machines, immutable audit/ledger behavior, idempotent writes, optimistic concurrency, and duplicate-event protection.
- [x] Prove empty-database migration, upgrade migration, deterministic seed, and documented roll-forward recovery against real ephemeral PostgreSQL.
- [x] Pass positive, negative, concurrency, capacity, idempotency, and same-transaction audit tests for every M3 state transition.
- [x] Save M3 command, API, test, and summary evidence under `docs/evidence/M03/` and pass the M3 gate.

### M4 — Authentication, authorization, and account lifecycle

- [x] Complete checkpoint `M04-account-lifecycle-local-001`: append-only account sessions and identity-binding lifecycle, single-use recent-auth grants, secure provider link/unlink, last-method and session-revocation rules, security notification fan-out, dual-controlled recovery holds, deletion/export requests, and real-PostgreSQL replay/concurrency proof without Entra or a phone.
- [x] Complete checkpoint `M04-account-read-ui-local-002a`: authenticated account-owned identity/session/request reads, privacy-safe response contracts, disabled/deletion-requested account denial, generated OpenAPI/client regeneration, explicit local/API mobile adapter, and generated-image-aligned Creator account UI without Entra or a phone.
- [x] Complete checkpoint `M04-account-mutations-local-002b`: provider-proof-gated identity link, recent-authenticated unlink, last-method protection, same-user session logout, export/deletion request routes, production/local proof separation, mutation-capable mobile adapters, local cache-clear coordination, and visible no-persistence UI controls without Entra or a phone.
- [x] Complete checkpoint `M04-mobile-auth-session-local-003`: one root mobile session context, native SecureStore boundary, memoryless browser preview, server-role route/resource matrix, Creator/Business mode switching with cache isolation, recent-auth state, logout purge, and visible iPhone-sized proof without Entra or a phone.
- [x] Complete checkpoint `M04-oidc-client-boundary-local-004`: production-fail-closed OIDC configuration, 256-bit PKCE/state/nonce transactions, protected one-use native storage, exact callback/deep-link validation, bounded cancellation/error handling, and visible no-provider local proof without Entra or a phone.
- [x] Complete checkpoint `M04-entra-verifier-local-005`: fail-closed external-tenant verifier configuration, pinned RS256 JWT verification, exact issuer/audience/tenant/version/scope/time checks, trusted bounded JWKS caching and key rotation, role/email exclusion, and generic failure proof using only local signing keys.
- [x] Complete checkpoint `M04-external-principal-mapping-local-006`: verified issuer/subject bearer evidence, active immutable identity resolution, explicit untrusted Creator/Business context, current profile/membership/workspace authorization, mobile role/workspace headers, fail-closed production activation, and unknown/revoked/invented/cross-tenant/disabled-state proof against local PostgreSQL.
- [x] Complete checkpoint `M04-mobile-code-exchange-local-007`: fail-closed token-endpoint boundary, locally signed RS256 ID-token and nonce verification, one-use code/PKCE proof, server-only role/workspace projection, memory-only access tokens, protected refresh rotation, strict restored-session validation, logout purge coordination, and cancellation/replay/wrong-code/wrong-verifier/wrong-nonce/expiry proof without a provider or phone.
- [x] Complete checkpoint `M04-server-session-bootstrap-local-008`: production-fail-closed bootstrap/refresh routes, 256-bit mobile session IDs, idempotent identity-bound session creation, safe Creator/Business context projection, session-bound external requests, generated client/mobile adapters, and PostgreSQL proof for collision, expiry, logout, identity/account/membership revocation, tenant-role isolation, and secret-safe logs without Entra or a phone.
- [x] Complete checkpoint `M04-mobile-auth-orchestration-local-009`: fail-closed sign-in/OIDC/exchange/bootstrap/refresh/logout orchestration, explicit progress/cancel/error/retry states, duplicate-tap protection, server-resolved multi-workspace selection, protected selected-context changes, and visible zero-provider iPhone-sized local preview proof without Entra or a phone.
...
- [x] Complete checkpoint `M05-terraform-resource-module-contract-local-002`: exact AzureRM provider/checksum lock, disposable resource-group/tag module, provider-independent workload safety contract, zero-change default plan, one-resource mock-provider plan, and refusal proof for environment, ownership, scale, network, backup, and anonymous-access violations without Azure credentials or execution.
- [x] Complete checkpoint `M05-workload-resource-modules-local-003`: conservative Storage, PostgreSQL, Container Apps, Service Bus, registry, Key Vault, and telemetry modules behind the disabled disposable root; zero-change default and exact 28-resource mock plans; identity/RBAC, secret/reference, image-digest, scale, network, backup, anonymous-access, tag, and retained/disposable safeguards without Azure execution.
- [x] Complete checkpoint `M05-secretless-oidc-plan-contract-local-004`: static immutable GitHub claim templates, three separate least-privilege plan/apply/destroy identities and environments, exact saved-plan command policy, four accepted invocations, twenty fail-closed cases, and an inactive workflow example while current CI remains non-deploying.
- [x] Complete checkpoint `M05-saved-plan-evidence-contract-local-005`: strict sanitized apply/destroy manifests, artifact and canonical review-payload SHA-256 binding, exact source/target/inventory/identity/command evidence, synthetic cost ceilings, independent review/approval, expiry and transient-deletion rules, and forty-three refusal cases without a Terraform plan binary or Azure execution.
- [x] Complete checkpoint `M05-ephemeral-run-ledger-contract-local-006`: 23-state apply/test/continue-or-rollback/destroy/reconcile ledger, three synthetic terminal paths, eleven required test gates, separate disposable state/live and retained inventories, attached timeout/orphan escalation, canonical digest binding, and fifty-nine refusal cases without cloud execution.
- [x] Complete checkpoint `M05-recovery-drill-contract-local-007`: canonical synthetic PostgreSQL PITR, Blob version/retention recovery, and Container Apps revision-restore evidence; draft RPO/RTO/timeout windows, seven-part migration/data/privacy/readiness reconciliation, attached timeout escalation, and sixty-two refusal cases without cloud recovery or provider contact.
- [x] Complete checkpoint `M05-dashboard-workload-module-local-008`: separate dashboard Container App and identity, ACR-pull-only RBAC, immutable image digest, HTTPS/CIDR-restricted ingress, non-secret runtime references, zero-to-one scale, exact 31-resource mock inventory, and rebound saved-plan/run-ledger evidence without Azure execution.
- [x] Complete checkpoint `M05-container-image-contract-local-009`: digest-required Node 24.19.0 base input, commit/time/version provenance, non-root API/dashboard/worker Dockerfiles, production-only offline bundles, Next.js standalone output, API/dashboard/worker runtime smoke, thirty-five refusals, and seven deferred image gates without Docker or registry execution.
- [x] Complete checkpoint `M05-local-preflight-audit-010`: reconcile all six M5 machine contracts, nine local preparation areas, active non-deploying CI, shared resource/expiry/identity boundaries, prohibited Terraform artifacts, sixteen separately approved external gates, and forty-five exercised refusals without Azure, registry, provider-backed planning, Terraform mutation, customer data, or cloud cost.
- [x] Complete checkpoint `M05-public-azure-service-cost-review-011`: select East US 2 from current official public service/region evidence, retain the seven low-cost workload service groups, capture exact USD retail meters, assign Blake Tindol as accountable platform/cost/alert/on-call/review owner, propose a conservative `$3.02` eight-hour estimate with a `$5` run ceiling and `$25` monthly budget, and prove twenty-six refusal cases without Azure authentication, subscription access, provider planning, registry contact, resource creation, or cost.
- [x] Complete checkpoint `M05-azure-naming-provider-scope-plan-012`: isolate Local Missions from the existing Azure workload with `rg-local-missions-*`/`lm` naming, unique `e2rYYMMDDNN` run stamps, four bounded cost tiers, hardened AzureRM provider behavior, read-only subscription/name checks, and one real-provider zero-resource scope plan using ephemeral test state without remote backend initialization, saved plan, apply, resource mutation, or cost.
- [x] Complete checkpoint `M05-retained-foundation-two-phase-plan-local-013`: add a three-resource retained-state bootstrap with immediate local-to-remote migration, a nine-resource retained identity/cost control plane with exact immutable GitHub subjects and Local Missions-tag-filtered budget, an applyable 28-resource workload-core phase, and a separately gated three-Container-App activation delta after immutable image publication; prove zero default changes and twenty refusal cases without Azure mutation.
- [x] Complete checkpoint `M05-shared-subscription-landing-zone-rbac-local-014`: replace subscription-scope stamped-resource-group recreation with one retained Local Missions landing zone, five group-scoped workflow assignments, and condition-limited role delegation for only five application data roles and ServicePrincipal recipients; prove a 15-resource retained control plane, 27-resource core, 30-resource activated workload, twenty placement refusals, and an eight-contract aggregate M5 preflight without Azure mutation.
- [x] Complete checkpoint `M05-workflow-delete-separation-local-015`: replace built-in Contributor on apply/destroy with two Local Missions-only custom roles at the retained landing-zone scope; apply can create/update but cannot delete, destroy can read/delete workload resources but cannot delete the landing-zone group, and constrained RBAC remains limited to five application data roles for ServicePrincipal recipients; prove a 17-resource retained control plane and twenty-two placement refusals without Azure mutation.
- [x] Complete checkpoint `M05-dedicated-bootstrap-scope-gate-local-016`: record the owner-selected dedicated subscription boundary, reject the only visible enabled subscription because read-only inventory found four non-Local-Missions resource groups, bind bootstrap planning to exact tenant/subscription IDs, require read-only dedicated-isolation proof and retained-cost/network inputs, and prove twenty-four Terraform refusals without provider registration, remote-backend initialization, saved plan, apply, deployment, or resource creation.
- [x] Complete checkpoint `M05-dedicated-subscription-created-017`: create the dedicated `Local Missions Development` subscription under the sole active Microsoft Customer Agreement billing scope and available standard Azure plan; verify it is enabled in the current tenant, has zero resource groups and zero resources, and grants the signed-in operator one subscription-level Owner assignment without creating resources, registering providers, changing the CLI default subscription, running Terraform, or starting later-phase planning.
- [x] Complete checkpoint `M05-dedicated-bootstrap-saved-plan-018`: detect and process-confine the current operator IPv4, bind AzureRM 5.0.1 to the exact dedicated subscription and tenant with automatic provider registration disabled, generate one protected saved plan containing exactly three retained-state creates, independently inspect its raw JSON, retain only sanitized digest/count/control evidence, and prove zero Azure resources, zero provider registration, zero apply, and no later-phase planning under the approved `$1/month` retained-state ceiling.
- [ ] Obtain separate approval before applying the reviewed three-resource bootstrap plan; after an approved apply, verify the resources and immediately migrate local bootstrap state to the Entra-backed remote backend. Separately approve the 17-resource control/landing-zone plan and constrained RBAC, monitored alert destination, `$25` workload budget, immutable GitHub subject preview, PostgreSQL administrator group, later trusted network rules, and Node base digest; then generate/review/apply the 27-resource core plan, publish immutable images, generate/review/apply the three-app activation plan, run cloud tests, and execute/reconcile the exact stamped resource destroy under separate approvals.
...
### 2026-08-27 — Mission contract and capacity boundary passed

- Milestone: M3 database, API contract, and domain state machines in progress
- Completed: Committed checkpoint `57139c0` with all four versioned mission-template codes, immutable campaign brief versions, objective checklist JSON, Community/Reach slot economics, fixed Reach bonus constraints, creator applications, active/converted/released reservations, and application status history. Campaign configuration now requires exact slot count, currency, Creator Reward Pool reconciliation, ordered slot identifiers, and at least 80% Community capacity; draft submission is blocked until that contract is complete. The Community application path has no follower field or Reach-provider dependency.
- Verification: Forward migration `0002_material_rachel_grey.sql` preserved a pre-existing `$575.00` campaign. Six new real-PostgreSQL tests passed no-follower schema inspection, invalid Community-mix rollback, one-creator duplicate race, six-qualified-creators-for-three-slots capacity race, withdrawal/replacement behavior, and cross-business acceptance denial. The combined M3 suite passes 15 tests with clean connection teardown; `pnpm verify` passes all workspace gates; the repeated seed retains four templates, one brief, and 10 Community Slots totaling `$500.00`; `db:check` verifies 16 tables; the security scan passes 254 text files; Gitleaks finds no leak in approximately 6.23 MB; and `drizzle-kit check` reports a consistent three-migration journal.
- Evidence: `docs/evidence/M03/mission-contract-capacity.md`, `docs/evidence/M03/test-results/mission-application-store-junit.xml`, `docs/evidence/M03/summary.md`, and `packages/db/drizzle/0002_material_rachel_grey.sql`.
- Decisions: At least `ceil(total slots × 0.80)` are Community Slots. Community eligibility uses approved status and current private locality verification, never follower count. Reach Slots are structurally separate and must use the fixed +50%, +100%, or +200% creator bonus, but Reach analytics qualification remains disabled and unimplemented. Withdrawal releases capacity for a replacement while the campaign/creator uniqueness rule still prevents repeat applications.
- Blockers: None for the next local slice. M3 remains open because check-in, submissions/media, disputes, payment ledger, Local Pass, consent/rights, notification/outbox, `/v1` API/OpenAPI clients, and remaining transition matrices are not implemented.
- Next exact task: Add accepted-mission scheduling plus venue-scoped check-in challenges/events and status history; prove expired, replayed, cross-creator, wrong-venue, unauthorized-staff, and duplicate check-ins fail without retaining raw coordinates.

### 2026-08-27 — Accepted-mission and check-in boundary passed

- Milestone: M3 database, API contract, and domain state machines in progress
- Completed: Committed checkpoint `49e6361` with one schedule per accepted application/slot, same-business venue validation, scoped Venue Staff assignments, rotating QR and staff-code challenges, hashed high-entropy tokens, five-minute server-time lifetimes, required staff fallback reasons, immutable check-in events, and assignment status history. Challenge consumption atomically verifies token, creator, venue, server window, expiry, and one-time state before committing the event, challenge consumption, `scheduled → checked_in` transition, history, and audit evidence.
- Verification: Forward migration `0003_orange_tempest.sql` preserved a pre-existing `$575.00` campaign. Eight new real-PostgreSQL tests passed privacy schema inspection, schedule/tenant isolation, unauthorized and authorized Venue Staff fallback, active QR rotation, successful privacy-minimized QR check-in, cross-creator/wrong-venue/server-expiry rejection, concurrent replay with exactly one winner, and future-window denial. The combined M3 suite passes 23 tests; `pnpm verify` passes all workspace gates; repeated seeding remains deterministic; `db:check` verifies 21 tables; the security scan passes 260 text files; Gitleaks finds no leak in approximately 6.40 MB; and `drizzle-kit check` reports a consistent four-migration journal.
- Evidence: `docs/evidence/M03/check-in-state-machine.md`, `docs/evidence/M03/test-results/check-in-store-junit.xml`, `docs/evidence/M03/summary.md`, and `packages/db/drizzle/0003_orange_tempest.sql`.
- Decisions: Venue Staff membership alone grants no check-in authority; location/window assignment is mandatory. PostgreSQL time controls challenge issuance and expiry. Plaintext tokens, latitude, longitude, and raw coordinates are absent; retained evidence is the verification method, a server-derived statement, and an `unavailable`, `coarse`, or `precise` accuracy class. Physical camera/location execution, rate limits, and suspicious-case queues remain later M9 work and are not claimed by this M3 checkpoint.
- Blockers: None for the next local slice. M3 remains open because submissions/media, review/revision, disputes, payment ledger, Local Pass, consent/rights, notification/outbox, `/v1` API/OpenAPI clients, and remaining transition matrices are not implemented.
- Next exact task: Add versioned deliverable requirements, direct-upload asset metadata/quarantine state, submission attempts, objective validation, one bounded correction, review decisions, and the 48-hour auto-approval deadline; prove missing/corrupt assets, pre-check-in submission, duplicate completion, unauthorized review, and second-revision attempts fail atomically.

### 2026-08-27 — Deliverable submission and review boundary passed

- Milestone: M3 database, API contract, and domain state machines in progress
- Completed: Committed checkpoint `a0402b3` with immutable ordered deliverable requirements bound to the accepted campaign brief, private media-object metadata and validation states, verified-check-in upload gating, all-requirements complete submission, exactly two possible submission attempts, one objective correction, tenant-scoped business approval, and 48-hour PostgreSQL-time auto-approval. Final approval locks and atomically completes the submission, application, assignment, and slot with matching history and audit rows.
- Verification: Forward migration `0004_handy_gideon.sql` preserved and backfilled a pre-existing checked-in assignment to its accepted brief. Seven new real-PostgreSQL tests passed privacy schema inspection, incomplete/out-of-range contract rollback, pre-check-in denial, missing/quarantined/invalid media rollback, duplicate-completion concurrency, unauthorized review plus one bounded correction, and server-time auto-approval concurrency. The combined M3 suite passes 30 tests; `pnpm verify` passes all workspace gates; repeated seeding retains two objective requirements; `db:check` verifies 29 tables; the security scan passes 266 text files; Gitleaks finds no leak in approximately 6.67 MB; and `drizzle-kit check` reports a consistent five-migration journal.
- Evidence: `docs/evidence/M03/submission-review-state-machine.md`, `docs/evidence/M03/test-results/submission-store-junit.xml`, `docs/evidence/M03/summary.md`, and `packages/db/drizzle/0004_handy_gideon.sql`.
- Decisions: Businesses can review only the accepted brief's structured objective criteria. A complete attempt starts one 48-hour window; one valid correction starts a second and final attempt with a fresh 48-hour window. Device time cannot extend either deadline. Metadata uses private storage keys and checksums rather than public URLs or file blobs; actual cloud upload intents and media processing remain later M10 work.
- Blockers: None for the next local slice. M3 remains open because disputes/resolutions, payment ledger/provider references, Local Pass, consent/rights, notification/outbox, `/v1` API/OpenAPI clients, latest-schema recovery proof, and remaining transition matrices are not implemented.
- Next exact task: Add disputes and resolutions tied to the final submission decision, with objective reason codes, evidence/audit timelines, tenant and creator authorization, admin-only resolution, one-open-dispute constraints, and atomic concurrency tests for duplicate, late, unauthorized, upheld, and overturned outcomes.

### 2026-08-27 — Dispute and all-or-nothing resolution boundary passed

- Milestone: M3 database, API contract, and domain state machines in progress
- Completed: Committed checkpoint `d673740` with timely business disputes during review, assigned-creator disputes against the single correction request, objective reason codes, two-to-ten structured mission-bound evidence references, one open dispute per assignment/submission/correction, a paused `disputed` submission state, and independent platform reviewer/admin resolution. Resolution is strictly `earned_full` or `no_payout`; it atomically finalizes mission records and creates exactly one immutable full-payable or full-refund financial-action intent without accepting a manual amount or calling a payment provider.
- Verification: Forward migration `0005_huge_agent_brand.sql` preserved a pre-existing approved submission and deterministically backfilled its full-payable financial intent. Eight new real-PostgreSQL tests passed schema/privacy inspection, unauthorized and subjective dispute rejection, incomplete and cross-mission evidence rollback, creator correction disputes, server-time deadline rejection, concurrent duplicate opening, approval-versus-dispute one-winner behavior, reviewer separation, and concurrent upheld/overturned resolution. The combined M3 suite passes 38 tests; `pnpm verify` passes all workspace gates; repeated seeding remains deterministic; `db:check` verifies 35 tables; the security scan passes 272 text files; Gitleaks finds no leak in approximately 6.95 MB; and `drizzle-kit check` reports a consistent six-migration journal.
- Evidence: `docs/evidence/M03/dispute-resolution-state-machine.md`, `docs/evidence/M03/test-results/dispute-store-junit.xml`, `docs/evidence/M03/summary.md`, and `packages/db/drizzle/0005_huge_agent_brand.sql`.
- Decisions: Subjective style, appearance, popularity, and follower-count objections are not valid dispute reasons. An active business owner/manager may open a timely objective dispute; only the assigned creator may dispute the correction request. The opener, creator, active business members, and ordinary staff cannot resolve the case. Approved work creates the full creator-payable intent; a valid no-payout outcome creates the full slot-refund intent. Processing fees, prorating, provider execution, and mutable money values are deliberately absent from this state machine.
- Blockers: None for the next local slice. M3 remains open because the immutable payment ledger/provider-reference model, Local Pass, consent/rights, notification/outbox, `/v1` API/OpenAPI clients, latest-schema recovery proof, and remaining transition matrices are not implemented.
- Next exact task: Create the immutable internal payment ledger and provider-reference model; consume full-payable and full-refund intents idempotently, enforce integer-minor-unit balanced entries and campaign/slot allocation constraints, and prove reconciliation, duplicate-event, unauthorized-adjustment, and concurrent-consumption behavior locally without Stripe calls or live money.

### 2026-08-27 — Immutable ledger and provider-reference boundary passed

- Milestone: M3 database, API contract, and domain state machines in progress
- Completed: Committed checkpoint `cd53b69` with an authoritative provider-funding boundary, immutable provider-object references, one frozen campaign funding snapshot, one fixed allocation per mission slot, deterministic integer-minor-unit allocation of the disclosed 15% platform fee, scoped ledger accounts, balanced append-only transactions/entries, and one-way consumption of full-payable/full-refund financial intents. Ordinary Business campaign transitions can no longer self-declare `funded`. Completion posts the full creator reward and only that slot's earned fee; no-payout posts the full reward-plus-fee Business refund obligation. Corrections are compensating journals requiring an active Finance Operator, reason, idempotency scope, and audit event.
- Verification: Forward migration `0006_dapper_mordo.sql` preserved a pre-existing pending full-payable intent. Nine new real-PostgreSQL tests passed schema/privacy inspection, canonical `$500 + $75 = $575` funding, business self-funding denial, deterministic rounding, duplicate/different-event races, concurrent full creator-payable posting, `$125 + $18.75 = $143.75` full refund posting, full Level 3 Reach reward/fee refund posting, unfunded/state-mismatch rollback, finance-role separation, immutable-record rejection, and deferred unbalanced-journal rejection. The combined M3 suite passes 47 tests; `pnpm verify` passes all workspace gates; repeated seeding remains deterministic; `db:check` verifies 41 tables; the security scan passes 278 text files; Gitleaks finds no leak in approximately 7.29 MB; and `drizzle-kit check` reports a consistent seven-migration journal. The retained local migration first exposed PostgreSQL's same-transaction new-enum rule; migration-only enum comparisons were made text-safe, the failed transaction left no partial schema, and the full migration then passed.
- Evidence: `docs/evidence/M03/immutable-ledger-provider-references.md`, `docs/evidence/M03/test-results/ledger-store-junit.xml`, `docs/evidence/M03/summary.md`, and `packages/db/drizzle/0006_dapper_mordo.sql`.
- Decisions: Provider references retain immutable synthetic IDs but no copied mutable status, secret, card, bank, tax, identity, or raw webhook payload. Funding debits provider clearing and credits campaign funds. Completion debits the whole slot allocation and credits creator payable plus platform fee revenue; no-payout credits the whole allocation to Business refund payable with no processing deduction. Posted intents and all provider/funding/allocation/ledger records reject mutation or deletion. No stored-value wallet, escrow claim, proration, Stripe API call, live payment, transfer, refund, or payout exists in this checkpoint.
- Blockers: None for the next local slice. M3 remains open because Local Pass, consent/rights, notification/outbox, `/v1` API/OpenAPI clients, latest-schema empty/recovery proof, raw-proof retention jobs, and remaining transition matrices are not implemented. Stripe webhook execution, provider status convergence, charges, transfers, refunds, payouts, chargebacks, reserve controls, and live-money approval gates remain M12.
- Next exact task: Add the core Local Pass transactional model: opaque campaign/creator pass links, venue-specific offer inventory, HMAC-style synthetic customer deduplication tokens, first-valid-claim attribution, short-lived hashed rotating claim tokens, future-claim pause, single-use Venue Staff redemption, explicit `pass_claimed` and `verified_pass_redemption` evidence, immutable histories/audits, and concurrency proofs for duplicate customer, final inventory, replay, wrong venue, unauthorized staff, and attribution overwrite—without sending SMS or using real customer data.

### 2026-08-27 — Local Pass transactional core passed

- Milestone: M3 database, API contract, and domain state machines in progress
- Completed: Committed checkpoint `8c21482` with one immutable venue offer per campaign, one hashed opaque link per completed creator assignment, first-valid creator attribution per campaign/customer, one seven-day pass claim, short-lived rotating claim-token hashes, inventory reservation/release, future-claim pause, exact-venue staff authorization, single-use redemption, equal-or-greater substitute rules, and explicit `link_open`, `pass_claimed`, and `verified_pass_redemption` evidence. Link opening, claiming, and redemption remain separate facts, and no Local Pass action changes creator reward, reliability, Reach, mission completion, or financial state.
- Verification: Forward migration `0007_thick_sharon_ventura.sql` preserved a pre-existing user and installed eight Local Pass tables. Five real-PostgreSQL tests passed schema/privacy/immutability checks, concurrent duplicate-customer attribution locking, final-inventory one-winner behavior, future-claim pause with active-pass preservation, rotating screenshot-token invalidation, wrong-venue and unauthorized-staff rejection, simultaneous redemption one-winner behavior, and unredeemed-expiry inventory release. The combined M3 suite passes 52 tests; `pnpm verify` passes all workspace gates; repeated seeding remains deterministic; `db:check` verifies 49 tables; the security scan passes 283 text files; Gitleaks finds no leak in approximately 7.66 MB; and `drizzle-kit check` reports a consistent eight-migration journal.
- Evidence: `docs/evidence/M03/local-pass-state-machine.md`, `docs/evidence/M03/test-results/local-pass-store-junit.xml`, `docs/evidence/M03/summary.md`, and `packages/db/drizzle/0007_thick_sharon_ventura.sql`.
- Decisions: Only token hashes and a versioned HMAC-style synthetic customer deduplication token are retained; phone, email, IP, device ID, and raw tokens are absent. Pausing blocks only future claims. A pass expires exactly seven days after claim and can be honored once only at its configured venue by an active owner/manager or currently assigned Venue Staff member. Evidence never claims a purchase, sale, revenue, or incremental lift. No SMS, customer account, external message, phone, payment provider, live money, Azure resource, or real customer data was used.
- Blockers: None for the next local slice. M3 remains open because consent/content-rights/disclosure records, notification/outbox, `/v1` API/OpenAPI clients, latest-schema empty/recovery proof, Local Pass OTP/recovery/refusal/reporting edges, raw-proof retention jobs, and remaining transition matrices are not implemented.
- Next exact task: Add the consent, disclosure, and content-license state model: persist accepted terms versions and covered assets/channels, activate the included 90-day organic or paid add-on license only after objective submission approval and the full creator-payable obligation, record expiry/renewal-ready audit history, and prove that canceled, incomplete, disputed-no-payout, refunded, or unpaid work grants no content-use rights—without external messaging, provider calls, or live money.

### 2026-08-27 — Content rights and fixed-license core passed

- Milestone: M3 database, API contract, and domain state machines in progress
- Completed: Committed checkpoint `a9b328a` with immutable hash-addressed Creator-terms and sponsorship-disclosure versions, exact assigned-Creator contract acceptance, one slot-level rights offer, and fixed licenses bound to the accepted brief, verified final assets, covered channels, compensation, permitted edits, start, expiry, and status history. The included 90-day organic license is always present after successful work; selected 12-month Business-owned media and 30-day paid-ad licenses add 50% and 100% of the base reward. A separate slot bonus-component migration allows transparent rights add-ons on Community Slots without turning them into follower-gated Reach Slots.
- Verification: Forward migrations `0008_fair_sheva_callister.sql` and `0009_nifty_scorpion.sql` preserved prior data and safely backfilled existing Reach bonuses. Six new real-PostgreSQL tests passed admin-only legal publication, immutable hash-only terms, fixed economics, exact-Creator consent and duplicate races, incomplete/unpaid/canceled/final-no-payout zero-rights behavior, concurrent idempotent activation, accepted-asset/channel enforcement, cross-Business denial, payment-state noninterference, no-content rejection, and server-time expiry without renewal. The combined M3 suite passes 58 tests; `pnpm verify` passes all workspace gates under pinned Node 24.19.0 and pnpm 11.24.0; repeated seeding remains deterministic; `db:check` verifies 56 tables; the security scan passes 290 text files; Gitleaks finds no leak in approximately 8.42 MB; and `drizzle-kit check` reports a consistent ten-migration journal.
- Evidence: `docs/evidence/M03/content-rights-license-state-machine.md`, `docs/evidence/M03/test-results/rights-store-junit.xml`, `docs/evidence/M03/summary.md`, `packages/db/drizzle/0008_fair_sheva_callister.sql`, and `packages/db/drizzle/0009_nifty_scorpion.sql`.
- Decisions: Rights activate only after final objective approval and the full Creator-payable obligation. Canceled, incomplete, unpaid, and final no-payout/refund-obligation work creates no license. Standard edits are only crop, resize, caption, logo placement, and minor formatting without misrepresentation. Permanent ownership, exclusivity, resale, third-party sublicensing, AI training, synthetic media, and face/voice cloning are absent. Expiry never auto-renews or backdates. No external message, provider call, live-money action, Azure resource, phone, or real customer data was used; final legal language still requires counsel review.
- Blockers: None for the next local slice. M3 remains open because notification preferences/outbox, `/v1` API/OpenAPI clients, latest-schema empty/recovery proof, Local Pass OTP/recovery/refusal/reporting edges, raw-proof retention jobs, Reach analytics qualification, rights-renewal operations, and remaining transition matrices are not implemented.
- Next exact task: Add transactional notification preferences and a domain-event outbox with idempotent delivery-attempt state and a no-send local adapter. Prove same-transaction event creation, duplicate worker claims, retry/backoff, opt-out, role/tenant isolation, retention-safe audit boundaries, and zero external delivery in local development.

### 2026-08-27 — Transactional notification outbox core passed

- Milestone: M3 database, API contract, and domain state machines in progress
- Completed: Committed checkpoint `811c8ea` with a fixed mission-action/reminder/money/dispute/security event catalog, versioned push/email preferences and immutable history, minimized immutable domain events, one transactional outbox message and durable in-app notice per event, leased worker claims, immutable delivery attempts/status history, exponential retry, dead letter, reasoned admin replay, and recipient-only inbox acknowledgment. Mission acceptance now creates its event, outbox, initial history, inbox notice, and notification audit in the exact transaction that accepts the application, reservation, and slot. The local/test worker adapter returns only `no_send` with `externalDeliveryAttempted: false` and refuses deployed environments.
- Verification: Forward migrations `0010_wide_lady_ursula.sql` and `0011_perpetual_ender_wiggin.sql` preserved prior data and installed seven notification tables. Seven new real-PostgreSQL tests passed minimized schema/immutability inspection, same-transaction acceptance fan-out plus forced rollback, concurrent event deduplication, recipient/role/tenant isolation, opt-out and duplicate-worker claims, local no-send outcomes, exponential backoff, stale-claim rejection, non-retryable dead letter, unauthorized/admin replay, and durable inbox acknowledgment. The combined M3 suite passes 65 tests; `pnpm verify` passes all workspace gates under pinned Node 24.19.0 and pnpm 11.24.0; repeated seeding remains deterministic; `db:check` verifies 63 tables; the security scan passes 300 text files; Gitleaks finds no leak in approximately 9.26 MB; and `drizzle-kit check` reports a consistent twelve-migration journal.
- Evidence: `docs/evidence/M03/notification-outbox-state-machine.md`, `docs/evidence/M03/test-results/notification-store-junit.xml`, `docs/evidence/M03/summary.md`, `packages/db/drizzle/0010_wide_lady_ursula.sql`, and `packages/db/drizzle/0011_perpetual_ender_wiggin.sql`.
- Decisions: Outbox intent and durable in-app state commit before any network work. Notification rows retain only a fixed template key, protected aggregate reference, recipient/tenant scope, deduplication key, and correlation ID—not phone, email address, device token, provider payload/response, location, reward, media URL, customer data, or free-form locked-screen copy. Required security notices cannot be opted out; ordinary channels can. Outbox status/counters/leases/replays are database constrained and history complete. No Azure queue, Expo token, provider, external message, phone, or real customer data was used.
- Blockers: None for the next local slice. M3 remains open because the `/v1` API/error/pagination/logging/dev-token boundary, reviewed OpenAPI and typed clients, latest-schema empty/recovery proof, Local Pass OTP/recovery/refusal/reporting edges, raw-proof retention jobs, Reach analytics qualification, rights-renewal operations, and remaining transition matrices are not implemented. Expo/email delivery, quiet hours, device-token lifecycle, real deep-link execution, and cloud dead-letter handling remain M14/M5 work.
- Next exact task: Build the `/v1` API foundation with standard validation/error and cursor-pagination contracts, request/correlation IDs, structured safe logs, live/ready/build endpoints, and a local/test-only dev-token endpoint that cannot activate in development, staging, or production. Generate and intentionally review the OpenAPI snapshot and typed mobile/dashboard client contract, then prove the boundaries with API integration tests without cloud resources or external identity calls.

### 2026-08-27 — Versioned API, OpenAPI, and shared typed-client foundation passed

- Milestone: M3 database, API contract, and domain state machines in progress
- Completed: Committed checkpoint `5428f43` with production and local NestJS/Fastify module graphs; standard safe errors; bounded request/correlation IDs; allowlisted structured request logs; process-only liveness, PostgreSQL-backed readiness, non-secret build info, and runtime OpenAPI; `/v1` discovery; and real stable cursor pagination over versioned mission templates. Local-only code issues a 15-minute process-ephemeral HMAC token for visibly synthetic identities, refuses every non-local environment, is absent from the production module and production OpenAPI, and is excluded and marker-scanned out of every clean production build. Deterministic production/local OpenAPI snapshots generate one `@local-missions/api-client` consumed by the iPhone app and dashboard; that client allows HTTPS plus loopback HTTP only.
- Verification: Seven API integration tests passed real-PostgreSQL live/ready/build behavior, two-page no-duplicate/no-missing template pagination, bounded validation errors, valid/invalid request-ID handling, safe-log field/query/body exclusion, production/local OpenAPI separation, production route absence, synthetic local-token issuance, deployed-environment refusal, and safe dependency failure. The complete integration gate passes 65 domain tests plus seven API tests. `pnpm verify` passes all nine-package gates under pinned Node 24.19.0 and pnpm 11.24.0; clean API builds contain no local-only source or marker; contract reconstruction matches both OpenAPI files and the generated client; repeated seed and `db:check` verify 63 tables; `drizzle-kit check` validates the twelve-migration journal; the security scan passes 332 text files; and Gitleaks finds no leak in approximately 9.40 MB. A compiled production-process smoke returned 200 for live, ready, build, V1, paginated templates, and OpenAPI, while the dev-token route returned the standard 404.
- Evidence: `docs/evidence/M03/api/contract-review.md`, `docs/evidence/M03/api/openapi.json`, `docs/evidence/M03/api/openapi.local.json`, `docs/evidence/M03/test-results/api-foundation-junit.xml`, `packages/api-client/src/generated/schema.ts`, and `docs/evidence/M03/summary.md`.
- Decisions: The committed/generated client follows only implemented production routes and cannot call local authentication. Request logs never retain query strings, bodies, headers, tokens, destinations, or provider payloads. Liveness does not depend on PostgreSQL; readiness does. Error envelopes never expose raw exceptions, SQL, hostnames, or credentials. Future retryable writes use the declared idempotency-key component, and contested edits use positive optimistic versions. No Entra, Azure, Stripe, provider, message, phone, live money, or real customer data was used.
- Blockers: None for the next local slice. M3 remains open because the latest complete schema needs one dedicated empty-database/previous-schema/failed-deployment roll-forward proof; Local Pass OTP/recovery/refusal/reporting edges, raw-proof retention jobs, Reach analytics qualification, rights renewal operations, authenticated domain HTTP resources, and remaining transition matrices are not implemented. Cloud identity, queues, storage, provider delivery, and live-money work remain behind later gates.
- Next exact task: Build the dedicated latest-schema migration and roll-forward recovery checkpoint. Prove all twelve migrations on a truly empty PostgreSQL database, prove an N-1 database with retained synthetic identity/campaign/history data upgrades without loss, inject a migration failure without partial commits, then apply the corrected forward migration and verify recovery. Add a migration-manifest/hash verifier and a production-safe runbook that forbids destructive down migrations, without Azure resources or external data.

### 2026-08-27 — Latest-schema migration and roll-forward recovery passed

- Milestone: M3 database, API contract, and domain state machines in progress
- Completed: Committed checkpoint `f1f61cd` with a deterministic manifest for all thirteen ordered Drizzle SQL files and snapshots, a contract gate that rejects artifact drift and unreviewed destructive statements, append-only migration `0012_notification_preference_history_backfill.sql`, and a forward-only operations runbook. Migration 0012 closes the real N-1 window in which preferences created after 0010 but before 0011 had no initial history row; it backfills each preference/version exactly once without changing existing migrations.
- Verification: Two dedicated real-PostgreSQL tests applied all thirteen migrations to a truly empty database, matched all 63 public tables and tracker hashes, retained a synthetic identity/Business/campaign/history/audit/preference fixture across an N-1 upgrade, injected a PostgreSQL failure after the final migration SQL but before tracker commit, proved complete transactional rollback, and then recovered forward with one version-1 backfill plus one trigger-created version-2 history row. The complete integration gate passes 67 database tests plus seven API tests. `pnpm verify` passes all nine workspace gates; repeated seed and `db:check` verify 63 tables; the security scan passes 342 text files; Gitleaks finds no leak in approximately 9.79 MB; and `drizzle-kit check` reports a consistent thirteen-migration journal.
- Evidence: `docs/evidence/M03/migration-roll-forward-recovery.md`, `docs/evidence/M03/test-results/migration-recovery-junit.xml`, `docs/evidence/M03/summary.md`, `docs/operations/migration-roll-forward.md`, `packages/db/drizzle/migration-manifest.json`, and `packages/db/drizzle/0012_notification_preference_history_backfill.sql`.
- Decisions: Applied migrations remain immutable and recovery always moves forward. Future staging/production migrations require one writer, reviewed hashes, a verified backup/PITR recovery point, readiness control, and row reconciliation. Operators may roll an application revision back only when it is explicitly compatible with the forward schema; they may not edit the migration tracker or run a destructive down migration. This checkpoint used only loopback PostgreSQL and synthetic data—no Azure resource, customer database, provider, real identity, phone, message, or live money.
- Blockers: None for the next local slice. M3 remains open because raw locality-proof retention, Local Pass OTP/recovery/refusal/reporting edges, Reach analytics qualification, rights-renewal operations, authenticated domain HTTP resources, and remaining transition matrices are not implemented. Azure backup/PITR and restore drills remain future infrastructure gates rather than claims of this local checkpoint.
- Next exact task: Add the raw locality-proof lifecycle locally: private synthetic evidence references, objective review/correction/appeal states, annual expiry and immediate invalidation on declared address change, a 30-day post-review-or-appeal deletion deadline, bounded expiring legal holds, idempotent worker claims/retries, evidence-reference clearing, immutable deletion/audit records, and failure alerts. Prove privacy, authorization, deadline, hold-expiry, duplicate-worker, and no-Community-assignment-when-expired behavior against PostgreSQL without storing a street address, raw document, real customer data, cloud blob, or external verifier.

### 2026-08-27 — Locality proof and 30-day retention control plane passed

- Milestone: M3 database, API contract, and domain state machines in progress
- Completed: Committed checkpoint `1ddb584` with six append-only locality tables and the `verification_reviewer` role. A Creator can submit one private proof reference, receive an objective correction/approval/rejection, resubmit once, and file a timely appeal decided by a different reviewer. Approval stores only the derived postal area and annual timestamps on the profile. A rejected renewal preserves a still-current credential during appeal; approval supersedes the prior credential. Declared address change invalidates locality immediately. Verification or appeal closure creates one 30-day deletion job with leased claims, bounded retry/dead-letter handling, immutable attempts, and one minimized alert. Legal holds require an administrator, fixed case/reason/scope, review date, and expiry within 90 days; expiry automatically makes due work claimable.
- Verification: Seven dedicated real-PostgreSQL tests passed prior-derived-credential backfill without raw proof, private response/audit boundaries, unauthorized and invalid-reason rollback, correction, independent appeal, annual expiry, immediate Community-application denial after address change, live/expired hold behavior, concurrent one-winner claims, synthetic reference clearing, immutable attempt/alert enforcement, and dead-letter evidence preservation. The updated recovery proof applies all fourteen migrations to an empty database and recovers forward after an injected final-migration failure. The complete integration gate passes 74 database tests plus seven API tests. `pnpm verify` passes all nine workspace gates; repeated seed and `db:check` verify 69 tables; the security scan passes 348 text files; Gitleaks finds no leak in approximately 10.31 MB; and `drizzle-kit check` reports a consistent fourteen-migration journal.
- Evidence: `docs/evidence/M03/locality-proof-retention-lifecycle.md`, `docs/evidence/M03/test-results/locality-proof-store-junit.xml`, `docs/evidence/M03/summary.md`, `packages/db/src/locality-proof-store.ts`, and `packages/db/drizzle/0013_brave_maddog.sql`.
- Decisions: The database stores no street/unit address, coordinate, document bytes, public URL, phone, email, bank, tax, payout, Stripe/KYC, or verifier payload. Businesses cannot read the private proof reference. The local/test synthetic deletion adapter clears only the reference and cannot activate in a deployed environment. Cloud Blob version/derivative erasure and backup aging remain future infrastructure gates; no Azure resource, external verifier, real document, real identity, phone, message, or live money was used.
- Blockers: None for the next local slice. M3 remains open because authenticated domain HTTP resources, Local Pass OTP/recovery/refusal/reporting edges, Reach analytics qualification, rights-renewal operations, and remaining transition matrices are not implemented. Device execution and cloud provider work remain deferred.
- Next exact task: Build the first authenticated domain HTTP vertical slice without a phone or cloud dependency: a production-safe bearer-verifier interface, local-only dev-token verification, `/v1/me` role/workspace context, Creator mission feed/detail/application endpoints, Business campaign read endpoints, tenant/role authorization, idempotent write handling, deterministic OpenAPI/client regeneration, and mobile data adapters that preserve the current generated-image UI with an explicit local fallback. Prove production/local auth separation, cross-role/cross-tenant denial, current-locality application gating, duplicate retries, pagination, safe errors/logs, and no Entra/Stripe/provider calls.

### 2026-08-27 — Authenticated Creator and Business HTTP slice passed

- Milestone: M3 database, API contract, and domain state machines in progress
- Completed: Committed checkpoint `1080db0` with a production-safe bearer-verifier interface, a fail-closed deployed implementation, and a separate process-ephemeral local signer/verifier. Added `/v1/me`, Creator mission feed/detail/application routes, Business campaign list/detail routes, current database role/workspace resolution, tenant isolation, stable cursor pagination, and same-transaction idempotent Community application retries. Regenerated the reviewed production/local OpenAPI artifacts and typed client. Added an explicit `local-preview`/`api` mobile adapter and wired the existing Creator feed/detail and Business dashboard without changing their generated-image visual structure.
- Verification: Five dedicated authenticated API tests passed production/local separation, current role/workspace resolution, invented-tenant rejection, cross-role denial, cross-tenant concealment, Creator feed/detail, two-page Business pagination, expired-locality denial, exact duplicate replay, changed-request key rejection, one committed application, and token-safe logs. The complete integration gate passes 74 database tests plus 12 API tests. `pnpm verify` passes all nine workspaces with 30 mobile tests; production builds exclude every local token marker; generated contracts match source; `db:check` verifies 69 tables; the security scan passes 357 text files; Gitleaks finds no leak in approximately 10.59 MB; and `drizzle-kit check` reports a consistent fourteen-migration journal.
- Evidence: `docs/evidence/M03/authenticated-domain-http-slice.md`, `docs/evidence/M03/test-results/authenticated-domain-api-junit.xml`, `docs/evidence/M03/api/contract-review.md`, `docs/evidence/M03/api/openapi.json`, `docs/evidence/M03/api/openapi.local.json`, and `packages/api-client/src/generated/schema.ts`.
- Decisions: One user selects Creator or Business context, but the server resolves every asserted role and Business public ID against current PostgreSQL state. Production authentication is intentionally unavailable until the later Entra integration gate and fails closed without an external call. Mobile defaults explicitly to `local-preview`; selecting `api` requires an authenticated session token and never silently falls back. No phone, Azure resource, Entra call, Stripe call, provider call, payment, message, or real identity/data was used.
- Blockers: None for the next local slice. M3 remains open because Local Pass OTP/recovery/refusal/reporting edges, rights-renewal operations, Reach analytics qualification, and remaining transition matrices are not implemented. Actual Entra, Stripe, cloud, and device execution remain later gated milestones.
- Next exact task: Add the Local Pass claim-edge checkpoint locally: short-lived challenge/OTP state, bounded attempts and resend limits, safe recovery, business refusal/substitution evidence, customer-visible fulfillment status, and privacy-minimized campaign reporting. Prove replay, brute-force, duplicate-customer, wrong-venue, refusal, recovery, and tenant-isolation cases without sending a real SMS/email or calling an external provider.

### 2026-08-27 — Local Pass claim edges passed

- Milestone: M3 database, API contract, and domain state machines in progress
- Completed: Committed checkpoint `d672439` with five-minute single-use customer challenges for claim, recovery, refusal reporting, substitute acceptance, and customer status access. Versioned customer/risk HMAC tokens are derived inside the store; only encrypted destination ciphertext and keyed OTP digests are retained. Verification locks after five failures, resends wait 60 seconds, and destination/link/risk sends are transactionally capped at three per 15 minutes. Claim and recovery proofs are consumed atomically, marketing consent remains separate and off, and contact/linkage deletion deadlines are persisted.
- Completed: Added verified refusal/incorrect-fulfillment evidence, independent Trust and Safety review, active-pass preservation, intentional/repeated-failure claim pausing, verified acceptance for non-preapproved substitutes, a privacy-minimized customer fulfillment view, and aggregate-only Business/Creator campaign reports with current tenant/creator authorization. Claims and verified redemptions remain separate evidence and are not labeled purchases, revenue, incremental customers, or lift. No outcome changes mission completion, creator reward, reliability, Reach, or financial state.
- Verification: Nine Local Pass real-PostgreSQL tests passed migration/privacy/immutability, duplicate-customer and final-inventory races, future-claim pause, OTP brute-force lockout, resend throttling, challenge replay, wrong-destination and fresh recovery, rotating-token replay, wrong venue/staff, refusal preservation/review, substitute acceptance, customer-safe status, aggregate reporting, cross-tenant denial, and expiry release. The complete integration gate passes 78 database tests plus 12 API tests. `pnpm verify` passes all nine workspaces with 30 mobile tests; repeated seed and `db:check` verify 72 tables; `drizzle-kit check` validates 15 migrations; the security scan passes 360 text files; and Gitleaks finds no leak in approximately 11.09 MB.
- Evidence: `docs/evidence/M03/local-pass-claim-edge.md`, `docs/evidence/M03/test-results/local-pass-claim-edge-junit.xml`, `packages/db/drizzle/0014_serious_terror.sql`, and `packages/db/drizzle/migration-manifest.json`.
- Decisions: Verification is a proof-of-control and abuse boundary, not marketing consent or an account. Existing passes survive future-claim pauses and confirmed refusals. Only Trust and Safety or an administrator can resolve a report; the affected Business cannot self-review. Contact ciphertext is scheduled for deletion after 30 days and dedup/linkage material after 12 months while aggregate evidence remains. This checkpoint used only synthetic loopback data and made no SMS, email, provider, phone, Azure, Entra, Stripe, payment, or external-record action.
- Blockers: None for the next local slice. M3 remains open because rights-renewal operations, Reach analytics qualification, and remaining transition matrices/audits are not implemented. The no-install Local Pass web/API and provider delivery are later M13 work; actual Entra, Stripe, cloud, and device execution remain later gated milestones.
- Next exact task: Add content-license renewal operations locally: Creator-visible renewal requests only within 30 days of expiry, fixed +25%/+50%/+100% rewards from the original locked base reward, explicit Creator accept/refuse with no reliability effect, separate Business funding intent, authoritative activation, new creator-payable obligation, non-backdating, no automatic renewal, and expiry/archive enforcement—without charging Stripe or extending rights before funding.

### 2026-08-27 — Authenticated Reach API and mobile data boundary passed

- Milestone: M3 database, API contract, and domain state machines in progress
- Completed: Committed implementation `5a23b49` with authenticated `GET /v1/creator/reach`, per-platform consent grant/revoke routes, and `GET /v1/business/reach-options`. Creator responses include only current consent, capability, and derived qualification state for the signed-in Creator. Business responses include the fixed Level 1/2/3 reward multipliers, the 80% Community floor, and platform booking availability—never raw counts, evidence references, provider connection references, combined audiences, or arbitrary follower filters. Regenerated and intentionally reviewed production/local OpenAPI and the shared typed client. Added explicit local/API Reach adapters and bound the Creator Reach and Business Budget screens to them without allowing local preview to persist consent or contact a provider.
- Verification: Fourteen API integration tests and 88 database integration tests passed real loopback PostgreSQL. The complete `pnpm verify` gate passed with 35 mobile tests and deterministic OpenAPI/client drift checks. Visible in-app-browser checks at `402 × 874` confirmed the Creator Reach view and the Business all-Community/mixed selection, including `$400 + $200 = $600`, a `$90` platform fee, `$690` Total Due, the 80% Community promise, disabled-provider copy, and no horizontal overflow.
- Evidence: `docs/evidence/M03/authenticated-reach-api.md`, `docs/evidence/M03/api/contract-review.md`, `docs/evidence/M03/api/openapi.json`, `docs/evidence/M03/api/openapi.local.json`, `packages/api-client/src/generated/schema.ts`, and `docs/evidence/M02/reach-ui-preview.md`.
- Decisions: Provider capability remains disabled independently for Instagram, TikTok, and YouTube. Recording optional consent does not imply a connection or qualification; Community access remains independent. Production bearer verification still fails closed until the later Entra gate. No provider, phone, Entra, Azure, Stripe, payment, message, live account, or real customer data was used.
- Blockers: None for the next local slice. M3 remains open because the broad schema/state-transition evidence checklist requires one final gap audit before the milestone gate can be claimed; live provider selection remains a later external feasibility/policy gate and does not block Community launch.
- Next exact task: Run the M3 completion gap audit against every unchecked M3 acceptance line, close only locally provable missing transition/evidence cases, regenerate the consolidated command/test summary, and pass the M3 gate without activating Azure, Stripe, Entra, or social providers.

### 2026-08-27 — M3 completion audit and milestone gate passed

- Milestone: M3 database, API contract, and domain state machines complete
- Completed: Added append-only migration `0017_charming_marrow.sql` with privacy-safe venue contacts, immutable contact status history, same-business scope enforcement, one active primary contact per location, explicit Local Pass attribution-confidence classifications, and a subject/type/time audit index. Added transactional create/revoke operations with tenant authorization and optimistic versions. Updated Local Pass event writes to persist exact observed/verified confidence without changing claims, redemption, rewards, or financial state.
- Verification: Four new venue-contact tests cover same-transaction history/audit, privacy schema inspection, cross-tenant and cross-scope rejection, direct-database scope enforcement, one-winner primary assignment, optimistic revoke races, and immutable rows/history. The Local Pass suite now asserts exact confidence classification and immutability. All 92 database integration tests and 14 API integration tests pass against real loopback PostgreSQL. All 18 migrations apply to an empty database and recover after an injected N-1 failure. Repeated seed plus `db:check` verify 89 tables. `pnpm verify` passes with 35 mobile tests and all nine builds; the security scan passes 390 text files; Gitleaks finds no leak in approximately 13.13 MB.
- Evidence: `docs/evidence/M03/m3-completion-audit.md`, `docs/evidence/M03/test-results/m3-complete-db-junit.xml`, `docs/evidence/M03/test-results/m3-complete-api-junit.xml`, `docs/evidence/M03/summary.md`, `docs/evidence/M03/commands.txt`, and `packages/db/drizzle/0017_charming_marrow.sql`.
- Decisions: M3 is a local transactional and contract proof, not external-provider execution. Real Entra authentication is M4; Azure, cloud media, Stripe execution/reconciliation, customer-facing Local Pass delivery, provider notifications, social-provider activation, and physical-iPhone gates remain in their named later milestones. No Azure, Entra, Stripe, social provider, notification provider, phone, payment, message, or external customer action occurred.
- Blockers: None for the next phone-free local slice. External Entra registration and real-device provider round trips remain explicit M4 gates and cannot be represented by synthetic proof.
- Next exact task: Begin `M04-account-lifecycle-local-001` with local account-session and provider-binding lifecycle records, recent-auth grants, safe provider link/unlink rules, last-method protection, session revocation, security notification events, recovery holds, and deletion/export requests. Prove anonymous/role/tenant boundaries and concurrent/replayed link/unlink behavior locally before any Entra registration or physical-phone round trip.

### 2026-08-27 — Local account lifecycle foundation passed

- Milestone: M4 authentication, authorization, and account lifecycle in progress
- Completed: Added append-only migration `0018_tiresome_wallflower.sql` with versioned active/revoked provider bindings, immutable identity history, identity-bound account sessions, bounded single-use recent-auth grants, controlled recovery holds/actions, and immutable export/deletion requests. Added a transactional account lifecycle store for secure provider link/unlink, last-method protection, removed-provider session revocation, security event/outbox/in-app fan-out, two-staff recovery-hold release, and recent-authenticated deletion requests. Legacy direct provider linking is blocked once the M4 schema exists.
- Verification: Five focused real-PostgreSQL tests passed privacy inspection, fresh/single-use link proof, replay rejection, same-transaction history/audit/notification evidence, last-method denial, concurrent dual-unlink one-winner behavior, session revocation, recovery hold enforcement, different-staff release, export/deletion requests, and immutable history. The full suite passes 97 database tests and 14 API integration tests. `db:check` verifies 96 tables; the 19-entry migration manifest and Drizzle journal pass; `pnpm verify` passes with 35 mobile tests and all nine builds; the security scan passes 397 text files; and Gitleaks finds no leak in approximately 13.78 MB.
- Evidence: `docs/evidence/M04/summary.md`, `docs/evidence/M04/commands.txt`, `docs/evidence/M04/test-results/account-lifecycle-junit.xml`, `packages/db/src/account-lifecycle-store.ts`, and `packages/db/drizzle/0018_tiresome_wallflower.sql`.
- Decisions: Email remains non-authoritative and absent from identity/session records. A provider binding is never transferred or hard-deleted. Unlinking must leave another verified method and revokes sessions tied to the removed method. Recovery holds block only sensitive new actions and do not erase earned rewards, refunds, ledger history, missions, or memberships. Account deletion is a request/state transition, not destructive local deletion.
- Blockers: M4 remains open for Entra app registrations/configuration, real issuer/audience/signature/scope validation, provider cancellation/consent/email-code cases, protected account HTTP/UI, logout/SecureStore cache purge, complete authorization matrix, and physical-device system-browser/deep-link round trips. No synthetic test can substitute for those external proofs.
- Next exact task: Build `M04-authenticated-account-api-002` locally: expose account identity/session/request reads and recent-authenticated link/unlink/request mutations behind the existing bearer boundary, ignore untrusted client role claims, reject disabled/deletion-requested users, preserve tenant/role isolation, regenerate OpenAPI/client contracts, and wire the existing account UI through an explicit local/API adapter without Entra or a phone.

### 2026-08-28 — Authenticated account read and Creator account UI passed

- Milestone: M4 authentication, authorization, and account lifecycle in progress
- Completed: Added authenticated `GET /v1/account` behind the existing bearer boundary. The server resolves the user and selected role from current PostgreSQL state, then returns only the account owner's active provider names/verification times, active session public IDs/times, recent account-request status, and sensitive-hold flag. Provider issuer/subject, email, address, bank, payout, and private locality evidence never enter the response. Regenerated the reviewed production/local OpenAPI artifacts and shared typed client.
- Completed: Added an explicit `local-preview`/`api` mobile account adapter and bound the Creator Account & Safety screen to it. The generated-image visual system now shows connected Apple/Google methods, active-session state, local-versus-authenticated labeling, and recovery-hold messaging. Local preview never contacts or simulates control of an identity provider, and its add-method action remains visibly disabled.
- Verification: Nine focused authenticated-domain tests and sixteen total API integration tests passed against real loopback PostgreSQL, including account ownership, decoy-user exclusion, safe-field exclusion, active-session reads, disabled/deletion-requested denial, current-role resolution, and existing cross-role/cross-tenant controls. All 97 database integration tests pass; `db:check` verifies 96 tables; `pnpm verify` passes all nine workspaces with 37 mobile tests and deterministic OpenAPI/client checks; the security scan passes 401 text files; and Gitleaks finds no leak in approximately 13.84 MB. The visible in-app browser confirmed the refreshed account UI at the iPhone breakpoint with no provider connection.
- Evidence: `docs/evidence/M04/summary.md`, `docs/evidence/M04/commands.txt`, `docs/evidence/M04/test-results/account-read-api-junit.xml`, `docs/evidence/M03/api/openapi.json`, `docs/evidence/M03/api/openapi.local.json`, and `packages/api-client/src/generated/schema.ts`.
- Decisions: Account reads are shared across Creator and Business modes because both roles belong to one root identity, but the selected role remains server-resolved and returned for clear UI context. Identity subjects and contact data are never account-screen fields. Production bearer verification remains fail-closed, and local preview is not evidence of provider control. No Entra, Azure, Stripe, social provider, phone, payment, message, real account, or customer data was used.
- Blockers: The broader `M04-authenticated-account-api-002` remains open for provider-proof-backed link, recent-authenticated unlink, session logout/revocation, export/deletion request HTTP mutations, cache/SecureStore purge, and mutation UI. Real provider proof and physical-device system-browser/deep-link execution remain external M4 gates.
- Next exact task: Build `M04-account-mutations-local-002b`: expose same-user session logout/revocation, export/deletion requests, and recent-authenticated unlink behind public-ID resolution; add a fail-closed provider-control-proof boundary before exposing link; wire the mutation UI without allowing local preview to persist changes; and prove replay, last-method, hold, disabled-account, role/tenant, and safe-cache behavior without Entra or a phone.

### 2026-08-28 — Authenticated account mutations and local proof boundary passed

- Milestone: M4 authentication, authorization, and account lifecycle in progress
- Completed: Added authenticated provider-link, provider-unlink, session-logout, and export/deletion request routes. Identity link accepts only an opaque provider-control proof plus a purpose-specific recent-auth grant; the app cannot submit provider issuer, subject, email, or ownership claims. Unlink resolves the current account's active provider server-side, requires fresh single-use authentication, preserves the last verified method, and revokes sessions tied to the removed provider. Logout resolves an active same-user public session ID, records an atomic audit, and rejects cross-account or replayed revocation. Deletion requires a current session plus recent authentication and immediately changes the root user to `deletion_requested`; export requires a current session but no destructive proof.
- Completed: Added an unavailable production provider-proof verifier and a separate five-minute, one-use synthetic local verifier/controller. The local signer requires an authenticated visibly synthetic account, is absent from production routes/OpenAPI/build output, and cannot instantiate outside `APP_ENV=local`. Regenerated production/local OpenAPI and the shared typed client.
- Completed: Extended the mobile account adapter with typed link/unlink/logout/export/deletion methods that never persist in `local-preview`. Added a logout coordinator that clears sensitive local account state even when remote revocation cannot be confirmed. Updated the generated-image-aligned Account & Safety and Account Deletion views with explicit sign-out, export, deletion-review, recent-auth, and no-local-mutation states; visible browser interaction confirmed both preview controls change state without a network/provider action.
- Verification: Six focused account-lifecycle tests and all 98 database integration tests pass. Eleven authenticated-domain tests and all 19 API integration tests pass, including provider-proof replay, last-method denial, same-user resolution, cross-account logout denial, logout replay, export, deletion-without-proof rejection, deletion state lockout, V1 account-resource discovery, safe duplicate-public-ID handling, safe logs, and production/local route separation. `pnpm verify` passes all nine workspaces with 41 mobile tests, deterministic OpenAPI/client checks, and production local-marker exclusion. `db:check` verifies 96 tables; the security scan passes 408 text files; and Gitleaks finds no leak in approximately 14.03 MB.
- Evidence: `docs/evidence/M04/summary.md`, `docs/evidence/M04/commands.txt`, `docs/evidence/M04/test-results/account-mutations-api-junit.xml`, `docs/evidence/M04/test-results/account-mutations-db-junit.xml`, `docs/evidence/M03/api/openapi.json`, `docs/evidence/M03/api/openapi.local.json`, and `packages/api-client/src/generated/schema.ts`.
- Decisions: Provider proof is a server-verifier result, never a client-authored provider identity. Synthetic local proof is test evidence only and cannot substitute for Entra or real provider control. Logout clears local sensitive state even if the server is unreachable, while surfacing the remote-revocation failure. Account requests are lifecycle records, not immediate destructive deletion/export execution. No Entra, Azure, Stripe, social provider, phone, payment, message, external account, or real customer data was used.
- Blockers: M4 remains open for the real mobile auth/session context and native SecureStore implementation, complete role/resource authorization matrix, Entra app registrations and issuer/audience/signature/scope validation, real Apple/Google/Microsoft/passwordless round trips, provider cancellation and email-code cases, and physical-device system-browser/deep-link proof.
- Next exact task: Build `M04-mobile-auth-session-local-003` without a phone: add a single root mobile auth/session context, native SecureStore-backed credential metadata with a nonpersistent web preview adapter, explicit Creator/Business mode switching from server-resolved roles, logout state/cache purge wiring, recent-auth-required UI state, and a route/resource authorization matrix. Prove cold start, expired/disabled/deletion-requested state, failed remote logout with local purge, role switch isolation, and local-preview no-token persistence before configuring Entra.

### 2026-08-28 — Root mobile auth/session and role-switch UI passed

- Milestone: M4 authentication, authorization, and account lifecycle in progress
- Completed: Added one app-root mobile auth/session provider with explicit restoring, anonymous, authenticated, expired, and blocked states. Cold-start restoration rejects expired, disabled, deletion-requested, malformed, or roleless protected state before private content is authorized. Runtime access tokens and five-minute purpose-specific recent-auth state are omitted from the persisted session projection.
- Completed: Added Expo SecureStore `57.0.2` with the config plugin and a device-bound `AFTER_FIRST_UNLOCK_THIS_DEVICE_ONLY` keychain boundary for future native refresh/session material. The web/local-preview adapter is intentionally memoryless: save is a no-op, load always returns null, and no token, identity, role, or session metadata enters browser storage.
- Completed: Added an explicit mobile route/resource authorization matrix for Creator missions, Business campaigns, Venue Staff check-in, and root account access. Private screens show a sign-in/account-state gate unless the server-resolved role is present. Mode changes increment the role-scoped cache epoch and never grant a missing role.
- Completed: Added a generated-image-aligned mode switcher to every Creator and Business shell, an explicit recent-sign-in state on Account & Safety, a safe memory-only preview entry on both sign-in screens, and working logout that returns home after local purge. The visible 390 × 844 browser flow switched Creator → Business, activated a five-minute nonpersistent recent-auth preview, signed out, and re-entered the Creator preview without a provider or network mutation.
- Verification: Thirteen focused auth/storage/logout tests and all 52 mobile tests pass. `pnpm verify` passes all nine workspaces, including formatting, prerequisites, lint, strict TypeScript, contracts, and builds. Expo config resolution and web export pass. Focused tests cover cold start, expiration, disabled/deletion-requested lockout, malformed/unreadable protected state, route/resource denial, unavailable mode rejection, purpose/expiry-bounded recent auth, native protected options, browser no-persistence, and local purge after both successful and failed remote revocation. The security scan passes 414 text files, Gitleaks finds no leak in approximately 14.08 MB, and screenshots were visually inspected at the iPhone-sized viewport with no critical horizontal clipping.
- Evidence: `docs/evidence/M04/summary.md`, `docs/evidence/M04/commands.txt`, `docs/evidence/M04/test-results/mobile-auth-session-junit.xml`, `docs/evidence/M04/screenshots/mobile-auth-creator-session.png`, and `docs/evidence/M04/screenshots/mobile-auth-business-mode.png`.
- Decisions: One root identity controls separate server-issued modes; the mode switch changes navigation and cache scope but never authorization. Browser preview state is deliberately disposable. SecureStore is wired for native protected material, but no real credential exists until Entra is configured and validated. Recent-auth proof remains memory-only and purpose-specific.
- Blockers: M4 still requires Entra registrations and exact issuer/audience/signature/scope validation; real Apple, Google, Microsoft, and passwordless-email round trips; PKCE system-browser/deep-link execution; provider cancellation/consent/email-code tests; and the deferred physical-iPhone gate. The current shell Node remains outside the pinned Node 24.19.x range and must be corrected before the final M4 gate.
- Next exact task: Build `M04-oidc-client-boundary-local-004` without a phone or provider call: add a production-fail-closed mobile OIDC configuration boundary, PKCE/state/nonce transaction lifecycle, exact callback/deep-link validation, cancellation/error mapping, and one-use return handling. Prove malformed/replayed/wrong-provider/wrong-redirect/expired returns locally before any Entra registration.

### 2026-08-28 — Mobile OIDC and PKCE client boundary passed

- Milestone: M4 authentication, authorization, and account lifecycle in progress
- Completed: Added a mobile OIDC configuration boundary that fails closed when values are absent, partial, insecure, or use the wrong redirect. Complete configuration requires the exact `localmissions://auth/callback` return, an HTTPS Entra authorization endpoint, a UUID client ID, and unique `openid`, `profile`, `offline_access`, and API `access_as_user` scopes.
- Completed: Added 256-bit independent verifier, state, and nonce generation; S256 PKCE; a ten-minute transaction; strict authorization-code request construction; and serialized one-use callback consumption. Native transaction storage protects the verifier with this-device-only SecureStore options, while browser/local preview uses memory only and persists no OIDC secret.
- Completed: Added exact scheme/host/path and allowlisted-query callback validation, expiration and replay denial, client-asserted-provider rejection, bounded cancellation/provider-error mapping, and safe generic error messages. Apple, Google, Microsoft, and passwordless email remain UI intent only until real Entra routing is configured; no provider selector or private callback description is trusted.
- Completed: Updated both Creator and Business sign-in views. At the iPhone breakpoint, selecting Google prepares the real local PKCE/state/nonce boundary and shows `Google request ready` while explicitly making no browser or provider request. The generated-image-aligned warm, navy, and teal visual system remains intact.
- Verification: Ten focused OIDC/storage tests and all 62 mobile tests pass. Mobile lint, strict TypeScript, resolved Expo config, web export, and the complete nine-workspace repository verification pass. The security scan passes 421 text files, and Gitleaks finds no leak in approximately 14.12 MB. A visible 390 × 844 browser run produced no provider-network request and the retained screenshot was visually inspected with no critical clipping.
- Evidence: `docs/evidence/M04/summary.md`, `docs/evidence/M04/commands.txt`, `docs/evidence/M04/test-results/oidc-client-boundary-junit.xml`, and `docs/evidence/M04/screenshots/oidc-pkce-request-ready.png`.
- Decisions: Provider intent never enters the authorization request or callback trust decision. State, nonce, verifier, authorization URL, and provider error descriptions are not rendered or logged. This checkpoint proves the client boundary only; it does not prove Entra registration, token exchange, JWT verification, provider routing, system-browser return, or a physical iPhone.
- Blockers: M4 still requires real Entra app/API registrations, exact issuer/audience/signature/algorithm/scope/time validation, Apple/Google/Microsoft/passwordless-email round trips, system-browser/deep-link execution, and the deferred physical-iPhone gate. The shell Node remains outside the pinned Node 24.19.x range and must be corrected before the final M4 gate.
- Next exact task: Build `M04-entra-verifier-local-005` without external setup: add fail-closed production JWT verifier configuration and locally signed fixture-key tests for issuer, audience, algorithm, signature, scope, expiration, not-before, nonce, JWKS cache/rotation, and safe failures. Keep real Entra registration and network verification for the later external gate.

### 2026-08-28 — Entra access-token verifier boundary passed locally

- Milestone: M4 authentication, authorization, and account lifecycle in progress
- Completed: Added a production-buildable Entra access-token verifier boundary using pinned `jose` 6.2.10. Configuration is unavailable when absent and rejects partial values, HTTP, credentials, query/fragment/port values, localhost/IP hosts, cross-host issuer/JWKS pairs, wrong tenant paths, wrong v2 issuer paths, wrong discovery-key paths, malformed audiences, and malformed required scopes.
- Completed: Added RS256-only JWT verification against a configured trusted JWKS resolver with exact issuer, API audience, external-tenant ID, v2 token version, required delegated scope, signature, key ID, `JWT` type, issued-at, not-before, and expiration checks. Token size, subject, scope count/shape, and maximum lifetime are bounded; header-supplied `jku`, `jwk`, `x5u`, and `x5c` key material is rejected.
- Completed: Added an HTTPS remote JWKS resolver with a three-second timeout, 30-second refresh cooldown, ten-minute maximum cache age, no redirect following, in-memory reuse, and unknown-key rotation reload. The verifier returns only issuer, external subject, tenant ID, token version, and scopes; token-carried role, email, and profile claims cannot grant app access and are not returned.
- Verification: Fourteen locally signed fixture tests pass configuration, valid identity evidence, wrong issuer/audience/tenant/version/scope, duplicate scope, expiration, not-before, future issued-at, algorithm confusion, attacker key location, wrong signature, unknown key, malformed/oversized token, cached reuse, key rotation, and generic no-secret errors. API lint, strict TypeScript, focused production build, and the complete nine-workspace repository verification pass. The security scan passes 424 text files, and Gitleaks finds no leak in approximately 14.14 MB.
- Evidence: `apps/api/src/entra-token-verifier.ts`, `apps/api/src/entra-token-verifier.test.ts`, `docs/evidence/M04/test-results/entra-token-verifier-junit.xml`, `docs/evidence/M04/summary.md`, and `docs/evidence/M04/commands.txt`.
- Decisions: Token verification and app authorization remain separate. This boundary does not trust an Entra token to name an internal user, Creator/Business role, or business workspace. The existing production API remains deliberately unavailable until external issuer/subject mapping and server-validated context selection are wired. Access-token validation does not pretend to prove the OIDC nonce; ID-token nonce validation remains part of the later mobile code-exchange/return gate.
- Blockers: M4 still requires external issuer/subject-to-root-user mapping, server-validated Creator/Business workspace selection, real Entra metadata/registration and network-key proof, mobile code exchange and ID-token nonce validation, provider round trips, system-browser/deep-link execution, and the deferred physical-iPhone gate. The shell Node remains outside the pinned Node 24.19.x range and must be corrected before final M4 proof.
- Next exact task: Build `M04-external-principal-mapping-local-006` without Entra or a phone: adapt the verified issuer/subject evidence into the bearer boundary, resolve only an active immutable external identity binding, accept an explicit mode/workspace request only as untrusted context, authorize it against current creator profile or business membership state, and prove unknown/revoked identity, invented role/workspace, cross-tenant, disabled account, and cache/session behavior with local PostgreSQL fixtures. Keep production activation fail-closed until real Entra configuration exists.

### 2026-08-28 — External principal and selected-context mapping passed locally

- Milestone: M4 authentication, authorization, and account lifecycle in progress
- Completed: Connected the AppModule bearer boundary to the strict Entra verifier only when complete safe configuration is present; absent configuration remains unavailable and partial configuration fails startup. Verified external tokens contribute only issuer/subject evidence. Each request resolves an active immutable external identity and current root account, then independently authorizes the requested Creator profile or exact Business membership/workspace from PostgreSQL.
- Completed: Added explicit untrusted `x-local-missions-role` and `x-local-missions-business` headers to the API/OpenAPI/client boundary. Invented roles, contradictory Creator/Business context, missing or cross-tenant workspaces, and membership-role elevation are denied generically. Mobile session code derives these headers only from the selected mode, server-returned roles, and selected workspace; account, mission, Reach, and logout requests share the same adapter boundary.
- Verification: Twenty-one focused real-PostgreSQL domain tests and all twenty-nine API integration tests pass. They prove same-identity Creator/Business selection, unknown/revoked non-enumeration, immediate identity revocation, disabled-account denial, disabled-membership denial, role elevation denial, workspace isolation, and token/header-safe structured logs. All 64 mobile tests, mobile lint/typecheck, API unit/typecheck, `db:check`, deterministic OpenAPI/client checks, and `pnpm verify` pass. The security scan passes 426 text files, and Gitleaks finds no leak in approximately 14.21 MB. An older OIDC test clock was made deterministic after the full suite exposed its fixed-date expiry. The disposable Creator App PostgreSQL container, volume, and network were destroyed; the unrelated Post Proof database container remained running.
- Evidence: `docs/evidence/M04/summary.md`, `docs/evidence/M04/commands.txt`, `docs/evidence/M04/test-results/external-principal-mapping-junit.xml`, `docs/evidence/M04/test-results/external-context-mobile-junit.xml`, `docs/evidence/M03/api/openapi.json`, `docs/evidence/M03/api/openapi.local.json`, and `packages/api-client/src/generated/schema.ts`.
- Decisions: A verified token never selects an internal role, profile, or tenant. Mode/workspace headers are explicit user context but remain untrusted until current database authorization succeeds. Membership, account, and identity revocations apply on the next request rather than waiting for a cached client role. No Entra/provider endpoint, real token, Azure resource, Stripe action, physical phone, payment, message, external account, or customer data was used.
- Blockers: M4 remains open for mobile authorization-code exchange, ID-token nonce validation, safe refresh-token rotation/revocation, real Entra registration/metadata/JWKS proof, Apple/Google/Microsoft/passwordless-email round trips, system-browser/deep-link execution, and the deferred physical-iPhone gate. The shell still runs Node 25.9.0 rather than the repository-pinned Node 24.19.x and must be corrected before final M4 proof.
- Next exact task: Build `M04-mobile-code-exchange-local-007` without a phone or provider call: add a fail-closed authorization-code exchange boundary, bind the one-use PKCE verifier and expected nonce to the return, validate a locally signed ID token without trusting email/role/profile claims, project the resulting access/refresh/session metadata into the existing protected mobile session lifecycle, and prove cancellation, replay, wrong nonce, wrong code/verifier, expired token, refresh rotation, logout revocation, and safe logs with local fixtures.

### 2026-08-28 — Mobile code exchange and protected refresh boundary passed locally

- Milestone: M4 authentication, authorization, and account lifecycle in progress
- Completed: Added a fail-closed token-endpoint interface and a pinned `jose` 6.2.10 signed ID-token verifier. RS256, trusted key resolution, exact issuer/mobile audience, type, key ID, issued-at, expiry, maximum age, subject, and the callback's expected nonce are required; token-header key locations are rejected. The consumed code and PKCE verifier are passed only to the exchange boundary, whose local fixture is one-use.
- Completed: Added server-session projection and refresh boundaries that ignore ID-token email/profile/role claims. Access tokens remain runtime-memory-only; a rotated refresh credential and non-secret session metadata persist through the device-bound SecureStore projection. Restored state now rejects unknown or duplicate roles, workspace-role contradictions, malformed public IDs, and malformed refresh material. Existing logout coordination revokes the Local Missions session when reachable and clears protected local state on both confirmed and unconfirmed revocation.
- Verification: Nine focused code-exchange tests and fifteen retained exchange/storage/logout tests pass locally signed cryptographic verification, cancellation/provider error short-circuiting, replay, wrong code/verifier, wrong nonce, expiry beyond intentional clock skew, server-only roles, access-token nonpersistence, refresh rotation, cross-session refresh denial, strict protected-state parsing, and logout purge. All 74 mobile tests, mobile lint/typecheck/build, and the complete nine-workspace `pnpm verify` gate pass. The security scan passes 429 text files, and Gitleaks finds no leak in approximately 14.25 MB. No logger receives credentials, and generic errors/persisted fixtures were inspected for token, provider-detail, and injected-role leakage.
- Evidence: `apps/mobile/lib/oidc-session-exchange.ts`, `apps/mobile/lib/oidc-session-exchange.test.ts`, `docs/evidence/M04/test-results/mobile-code-exchange-junit.xml`, `docs/evidence/M04/summary.md`, and `docs/evidence/M04/commands.txt`.
- Decisions: The ID token proves the nonce-bound authentication return but never grants an app role or workspace. The Local Missions server session projection remains the only source of account/provider/role/workspace state. Browser preview never persists credentials. The default token endpoint is deliberately unavailable until reviewed HTTPS provider transport and configuration exist.
- Blockers: M4 remains open for a server bootstrap/refresh API, Local Missions session binding on every external bearer request, real Entra token transport/metadata/JWKS proof, Apple/Google/Microsoft/passwordless-email round trips, system-browser/deep-link execution, and the deferred physical-iPhone gate. The shell still runs Node 25.9.0 rather than pinned Node 24.19.x and must be corrected before final M4 proof.
- Next exact task: Build `M04-server-session-bootstrap-local-008` against ephemeral PostgreSQL: add a production-fail-closed bootstrap/refresh API that maps only verified external evidence, creates or refreshes one active identity-bound Local Missions session idempotently, returns safe available Creator/Business contexts, requires that session on later external bearer requests, and proves replay, collision, revoked/expired session, identity unlink, account disablement, role/workspace isolation, logout invalidation, and secret-safe logs without contacting Entra.

### 2026-08-28 — Identity-bound Local Missions session bootstrap passed locally

- Milestone: M4 authentication, authorization, and account lifecycle in progress
- Completed: Added production-buildable `POST /v1/session/bootstrap` and `POST /v1/session/refresh` routes. Only verified external issuer/subject evidence can reach them; missing production Entra configuration remains fail-closed. Bootstrap accepts a phone-generated 256-bit opaque session public ID, creates or reuses exactly one active session bound to the same root user and external identity, and returns only active account status, provider label, safe Creator/Business roles, and current Business workspace names/public IDs/owner-or-manager roles.
- Completed: Every later external bearer request now also requires `x-local-missions-session`. The server rechecks the active identity, same-user/same-identity session, session expiry/revocation, root account, Creator profile, and exact Business membership/workspace on each request. Mobile account, mission, Reach, and logout adapters emit the bound session header; bootstrap/refresh use the generated client without persisting an access token or provider subject.
- Verification: Pinned Node 24.19.0 and pnpm 11.24.0 passed the complete nine-workspace `pnpm verify` gate. All 99 database integration tests and 33 API integration tests passed; focused retained evidence contains 25 API, seven account-lifecycle PostgreSQL, and 32 mobile tests with zero failures/errors. Mobile passed all 77 tests, deterministic seed and `db:check` verified 96 tables, the final security scan passed 435 text files, and Gitleaks found no leak in approximately 14.39 MB. The visible Local Missions preview remained on the 390 × 844 Business dashboard with no console errors. The disposable Creator App PostgreSQL container, volume, and network were destroyed while the unrelated Post Proof database remained running.
- Evidence: `docs/evidence/M04/test-results/server-session-bootstrap-api-junit.xml`, `docs/evidence/M04/test-results/server-session-bootstrap-db-junit.xml`, `docs/evidence/M04/test-results/server-session-bootstrap-mobile-junit.xml`, `docs/evidence/M04/summary.md`, `docs/evidence/M04/commands.txt`, `docs/evidence/M03/api/openapi.json`, `docs/evidence/M03/api/openapi.local.json`, and `packages/api-client/src/generated/schema.ts`.
- Decisions: Provider access tokens identify external subjects but never become Local Missions sessions, roles, or workspaces. A separate opaque Local Missions session proves the app session and makes logout, identity unlink/revocation, account disablement, session expiry, and membership removal effective immediately even if a provider access token remains valid. Bootstrap is idempotent only for the same active user/identity/session tuple; collisions and stale sessions are never revived. No Entra/provider endpoint, real token, Azure resource, Stripe action, physical phone, payment, message, external account, or customer data was used.
- Blockers: M4 remains open for reviewed real Entra app/API registration and configuration, HTTPS token transport and network JWKS proof, Apple/Google/Microsoft/passwordless-email round trips, multi-workspace selection UI, system-browser/deep-link execution, provider cancellation/consent/email-code cases, and the deferred physical-iPhone gate. This local checkpoint does not claim any external-provider or physical-device proof.
- Next exact task: Build `M04-mobile-auth-orchestration-local-009` without contacting a provider: connect the existing sign-in, OIDC transaction, code-exchange, server-bootstrap, protected-session, refresh, and logout boundaries behind fail-closed configuration; add explicit loading/cancel/error/retry and multi-workspace selection states; and prove duplicate taps, unavailable configuration, cold-start refresh, selected-context changes, and zero-network local preview behavior before real Entra setup.

### 2026-08-28 — Mobile authentication orchestration passed locally

- Milestone: M4 authentication, authorization, and account lifecycle in progress
- Completed: Connected one mobile sign-in orchestrator across protected OIDC transaction creation, system-browser result handling, code exchange, ID-token verification, server-session bootstrap, protected refresh, session selection, and existing logout purge. The checkpoint runtime is deliberately configuration-unavailable, so API mode fails before opening a browser or contacting a token endpoint until reviewed external transport is supplied.
- Completed: Added preparing, waiting, exchanging, cancelled, bounded error, retry, local request-ready, workspace-required, and authenticated UI states. Duplicate taps are ignored while one browser attempt is active. Multi-workspace server projections retain every safe workspace and require an explicit authorized choice instead of defaulting to the first; the selected workspace now appears on the Business dashboard.
- Verification: Thirty-three retained orchestration/session/storage/exchange/projection tests and all 84 mobile tests pass. Pinned Node 24.19.0 and pnpm 11.24.0 passed `pnpm verify` across all nine workspaces. The security scan passed 439 text files, and Gitleaks found no leak in approximately 14.44 MB. At 390 × 844, Creator Apple preparation stayed on localhost with no provider tab, Business preview displayed two synthetic workspaces, and selecting Lake Eola Cafe produced the matching dashboard greeting.
- Evidence: `apps/mobile/lib/auth-orchestration.ts`, `apps/mobile/lib/auth-orchestration-runtime.ts`, `apps/mobile/lib/auth-orchestration.test.ts`, `docs/evidence/M04/test-results/mobile-auth-orchestration-junit.xml`, `docs/evidence/M04/summary.md`, and `docs/evidence/M04/commands.txt`.
- Decisions: Provider configuration remains fail-closed and no external call is possible from the checkpoint runtime. More than one Business workspace requires explicit choice; a single server-returned workspace may continue directly. Local preview stays memory-only and does not claim provider, token, native-browser, or physical-device proof.
- Blockers: M4 remains open for user-controlled Entra tenant/app/API registration, reviewed HTTPS token transport and network JWKS proof, Apple/Google/Microsoft/passwordless-email provider round trips, consent/cancellation/email-code handling against real providers, native system-browser/deep-link execution, and the deferred physical-iPhone gate. No external provider, Azure resource, real identity, token, payment, or customer data was used.
- Next exact task: Prepare `M04-external-auth-configuration-gate-010` without creating resources or contacting a provider: document the exact Entra tenant/app/API registration inputs, redirect/provider mappings, secret-free environment contract, reviewed HTTPS token/JWKS transport boundaries, consent/cancellation/email-code test matrix, and separate approval points. Keep physical-iPhone execution deferred until the user is ready.

### 2026-08-28 — External authentication configuration gate passed locally

- Milestone: M4 authentication, authorization, and account lifecycle in progress
- Completed: Added a versioned machine-readable activation manifest and operational runbook for separate mobile public-client, API-resource, and deferred participant-web registrations. The contract fixes the exact `localmissions://auth/callback` return, authorization-code + S256 PKCE grant, `access_as_user` API scope, Apple/Google/Microsoft/passwordless-email hosted-flow mappings before role selection, explicit Facebook/Meta exclusion, environment separation, and seven separately owned approval points.
- Completed: Reconciled the checked-in mobile/API `.env.example` files with the environment names actually read by the fail-closed parsers. Mobile configuration now requires the authorization, token, issuer, and JWKS endpoints on one HTTPS origin and tenant path, a UUID public-client ID, the exact callback, and exactly the four reviewed scopes. No mobile client/provider secret is permitted. API examples now use the exact tenant/audience/issuer/JWKS/scope names accepted by the verifier.
- Completed: Added `pnpm external-auth:check` to the full repository gate. It validates three registration records, four V1 providers, twelve public/identifier environment fields, four transport boundaries, nine provider/device cases, and seven unapproved approval gates. It also proves the mobile example and parser stay synchronized, activation remains `planned_not_activated`, the real iPhone row remains deferred, and no external pass is claimed.
- Verification: Pinned Node 24.19.0 and pnpm 11.24.0 passed 24 focused mobile OIDC/orchestration/exchange tests, 14 focused API verifier tests, all 84 mobile tests, formatting, prerequisite and external-auth validation, lint, strict type checks, every repository unit test, contract checks, production marker checks, and all nine builds. The security scan passed 444 text files, and Gitleaks found no leak in approximately 14.49 MB. Retained JUnit reports contain 38 tests with zero failures/errors and a sanitized local hostname.
- Evidence: `config/external-auth-gate.v1.json`, `docs/operations/external-auth-configuration-gate.md`, `scripts/validate-external-auth-gate.mjs`, `docs/evidence/M04/test-results/external-auth-config-mobile-junit.xml`, `docs/evidence/M04/test-results/external-auth-config-api-junit.xml`, `docs/evidence/M04/summary.md`, and `docs/evidence/M04/commands.txt`.
- Decisions: This checkpoint is a reviewed handoff, not external authorization or activation. The same hosted customer flow serves every sign-in provider before Local Missions resolves Creator/Business/Venue Staff roles. Provider intent, email, token-carried role, and selected phone mode remain untrusted. The current runtime still cannot exchange or refresh a provider token, and the local gate does not claim Entra tenant/app registration, provider credentials, external JWKS traffic, a real identity, or a physical iPhone.
- Blockers: M4 remains open for explicit owner-approved Entra tenant/app/API mutations, provider terms/credential entry, production-buildable mobile HTTPS token/JWKS transport, real issuer/audience/network-key proof, provider consent/cancellation/email-code round trips, native system-browser/deep-link execution, the complete final authorization matrix, and the deferred physical-iPhone gate.
- Next exact task: Build `M04-mobile-auth-transport-local-011` without contacting a provider: add a production-buildable, dependency-injected HTTPS token exchange/refresh adapter and mobile JWKS resolver with exact same-origin endpoints, form-encoded public-client requests, no client secret, rejected redirects, bounded timeout/content type/body size/cache/rotation, generic no-secret failures, and local fixtures. Keep the installed runtime disabled until the external configuration and activation approvals are explicitly granted.

### 2026-08-28 — Mobile authentication transport passed locally

- Milestone: M4 authentication, authorization, and account lifecycle in progress
- Completed: Added a production-buildable `HttpsOidcTokenEndpoint` for authorization-code + S256 PKCE exchange and refresh rotation. It revalidates the complete OIDC configuration, posts only to the exact HTTPS token endpoint with `application/x-www-form-urlencoded`, sends the public mobile client ID but no client secret, omits ambient credentials/referrer data, rejects redirects/followed redirects, uses a ten-second abort boundary, requires bounded JSON, and returns only the allowlisted token-set fields.
- Completed: Added a bounded mobile JWKS resolver using pinned `jose`. It fetches only the exact same-tenant HTTPS JWKS URI with no ambient credentials/referrer, rejects redirects and non-JSON/error/oversized/malformed/empty/overlarge key sets, limits the body to 64 KiB and twenty keys, uses a three-second timeout, ten-minute cache, thirty-second cooldown, and reloads for an unknown rotated key. All provider/network/body failures become existing generic sign-in errors without logging request or response bodies.
- Completed: Strengthened mobile OIDC configuration to reject localhost, IP literals, non-UUID tenant paths, cross-origin endpoints, and mixed tenant paths. Updated the machine-readable external-auth contract/runbook and validator to record both local transport implementations while proving the installed runtime still instantiates only `UnavailableOidcTokenEndpoint` and no mobile JWKS resolver.
- Verification: Twenty-seven focused fixture-backed transport tests pass exact exchange/refresh bodies, no-secret/no-credential headers, mismatched input rejection before fetch, status/content-type/body-size/JSON/schema/followed-redirect/timeouts, refresh-specific failures, local signed-key verification, cache reuse, key rotation, JWKS bounds, and a concrete token + JWKS + ID-token + server-bootstrap + protected-session composition. All 111 mobile tests pass. Pinned Node 24.19.0 and pnpm 11.24.0 passed formatting, prerequisite/external-auth validation, all nine-workspace lint/typecheck/tests/contracts/builds, the security scan over 447 text files, and Gitleaks over approximately 14.53 MB.
- Evidence: `apps/mobile/lib/oidc-network-transport.ts`, `apps/mobile/lib/oidc-network-transport.test.ts`, `docs/evidence/M04/test-results/mobile-auth-transport-junit.xml`, `config/external-auth-gate.v1.json`, `docs/operations/external-auth-configuration-gate.md`, `scripts/validate-external-auth-gate.mjs`, `docs/evidence/M04/summary.md`, and `docs/evidence/M04/commands.txt`.
- Decisions: The new adapters are production code but not runtime activation. The checked-in app still fails before browser/token/JWKS work because its orchestrator uses unavailable configuration, unavailable token transport, and unavailable ID-token verification. Dependency-injected synthetic responses and locally generated signing keys prove behavior without contacting Entra or any provider. No external registration, network identity request, real identity, or physical-device proof is claimed.
- Blockers: M4 remains open for the consolidated final authorization matrix, explicit owner-approved Entra tenant/app/API registration and runtime activation, provider terms/credential entry, real issuer/audience/network-key proof, Apple/Google/Microsoft/passwordless-email consent/cancellation/code round trips, native system-browser/deep-link execution, and the deferred physical-iPhone gate.
- Next exact task: Build `M04-authorization-matrix-audit-local-012` without Entra or a phone: reconcile every M4 authorization-matrix row against current API/database/mobile evidence, add any missing participant, cross-business, Venue Staff, support, admin-reason/audit, disabled-user propagation, untrusted-role, identity-link collision, last-method replay, unlink notification, and dual-control recovery-hold proofs against synthetic fixtures and ephemeral PostgreSQL, then record only the locally proven rows while leaving external/provider/device gates open.

### 2026-08-28 — Consolidated authorization matrix passed locally

- Milestone: M4 authentication, authorization, and account lifecycle in progress
- Completed: Added a database-derived authorization policy for Creator ownership, Business tenant isolation, active Venue Staff assignment, bounded Support investigation, separate Finance mutation authority, and reasoned Admin overrides. Caller-shaped roles are ignored, disabled users are rejected immediately, and unauthorized ownership/tenant reads use a bounded concealed response.
- Completed: Added append-only migration `0019_ambiguous_kate_bishop.sql`, bringing the reviewed manifest to 20 migrations while adding the distinct `support_agent` role and database-enforced immutable audit events. Migration recovery now proves the enum/trigger are absent after an injected latest-migration rollback and present after forward recovery.
- Completed: Strengthened provider-link collision, replayed/concurrent unlink, owner notification, and total-lockout recovery behavior. Recovery holds revoke active sessions, prevent new sessions and sensitive money/account actions, notify the user, and require a different authorized staff member for release.
- Verification: The machine validator records 12 of 12 local rows as proven and keeps five external/native gates open. Focused JUnit reports retain 15 PostgreSQL and 30 API tests with zero failures/errors. Full regressions pass 107 database tests across 16 files and 38 API tests across two files. A fresh disposable PostgreSQL volume verified 96 tables and 20 migration hashes under pinned Node 24.19.0 and pnpm 11.24.0. The complete repository gate passed, the security scan passed 456 text files, and Gitleaks found no leaks in approximately 15.18 MB.
- Evidence: `config/authorization-matrix.v1.json`, `docs/evidence/M04/authorization-matrix.md`, `docs/evidence/M04/test-results/authorization-matrix-db-junit.xml`, `docs/evidence/M04/test-results/authorization-matrix-api-junit.xml`, `docs/evidence/M04/summary.md`, and `docs/evidence/M04/commands.txt`.
...
### 2026-08-28 — Terraform resource-module contract passed locally

- Milestone: M5 Azure dev foundation and continuous delivery in Phase A local preparation; all Azure execution remains deferred
- Completed: Pinned `hashicorp/azurerm` exactly to 5.0.1 and committed its signed package checksums. Added one `rg-local-missions-dev-*` resource-group/tag module behind the disabled root plus a provider-independent workload contract for development isolation, retained/disposable ownership, zero-to-one scale defaults, TLS/firewall bounds, seven-day point-in-time recovery, and private non-anonymous Blob access.
- Completed: Extended the machine contract and scrubbed validator to admit only `azurerm_resource_group`, require no real `provider "azurerm"` block, prove the ordinary fixture remains zero-change, and inspect the separate mock-only create. Added five safety refusal fixtures to the existing target/expiry/activation refusals.
- Verification: Backend-disabled validation and Terraform mock-provider planning passed for two roots and three plan tests. Eight refusal tests passed, the default plan had zero resource changes, and the mock-only plan had exactly one `azurerm_resource_group` create. Eleven workload tags, twelve safeguards, ten low-cost defaults, four expiration fixtures, and eight external gates remained machine-checked. TFLint 0.63.1 reported zero findings across both roots and the recursive dev modules. The complete pinned-runtime verification passed all nine workspaces and 111 mobile tests; the security scan passed 496 text files, and Gitleaks found no leaks in approximately 15.27 MB.
- Evidence: `infra/terraform/modules/resource-group/`, `infra/terraform/modules/workload-contract/`, `infra/terraform/environments/dev/.terraform.lock.hcl`, `config/terraform-foundation.v1.json`, `scripts/validate-terraform-foundation.mjs`, `docs/evidence/M05/summary.md`, and `docs/evidence/M05/commands.txt`.
- Decisions: The AzureRM package was resolved from Terraform Registry only to lock code and checksums. No Azure provider configuration, credential, subscription, remote backend, refresh, live plan, resource, networking activation, apply, destroy, or cost was used. Candidate region/SKU/price values remain unapproved.
- Blockers: M5 remains open for additional reviewed workload resource modules, secretless OIDC, current cost/service validation, saved live-plan review, explicit apply approval, cloud verification, rollback/restore, scoped destroy, and independent teardown reconciliation. The user has explicitly deferred every Terraform execution against Azure.
- Next exact task: Build `M05-workload-resource-modules-local-003` as mock-only local code: add conservative Storage, PostgreSQL, Container Apps, Service Bus, registry, Key Vault, and telemetry module contracts behind the disabled root, then prove their tags, scale, network, backup, and anonymous-access behavior with synthetic mock plans. Do not configure Azure credentials, initialize a remote backend, contact a subscription, produce a provider-backed plan, apply, destroy, or treat any candidate SKU/price as approved.

### 2026-08-28 — Workload resource modules passed locally

- Milestone: M5 Azure dev foundation and continuous delivery in Phase A local preparation; all Azure execution remains deferred
- Completed: Added conservative Storage, PostgreSQL, Container Apps, Service Bus, Container Registry, Key Vault, and telemetry modules behind the disabled disposable root. The exact mock inventory is 28 resources across 17 reviewed types, including three private Blob containers, Entra-only PostgreSQL, separate API/worker managed identities, eight scoped role assignments, a duplicate-detecting queue, RBAC-only Key Vault with no Terraform-managed secrets, and identity-only service access where the reviewed provider contracts support it.
- Completed: Added immutable image-digest, complete synthetic identity-reference, and zero-secret/reference contracts plus three new refusal fixtures. Every taggable resource retains the eleven exact disposable tags; default plans remain zero-change; the mock-only plan is create-only; and retained-resource ownership remains outside the workload root.
- Verification: Backend-disabled validation and all 3 plan tests passed. All 11 refusal tests passed, and the enabled mock plan contained exactly 28 create-only changes across 17 reviewed resource types. TFLint 0.63.1 reported zero findings across both roots and all recursive modules. The complete pinned Node 24.19.0/pnpm 11.24.0 repository gate passed all nine workspaces and 111 mobile tests; the security scan passed 534 text files; and Gitleaks found no leaks in approximately 15.35 MB.
- Evidence: `infra/terraform/modules/workload-storage/`, `infra/terraform/modules/workload-postgresql/`, `infra/terraform/modules/workload-container-apps/`, `infra/terraform/modules/workload-service-bus/`, `infra/terraform/modules/workload-registry/`, `infra/terraform/modules/workload-key-vault/`, `infra/terraform/modules/workload-telemetry/`, `config/terraform-foundation.v1.json`, `scripts/validate-terraform-foundation.mjs`, `docs/evidence/M05/summary.md`, and `docs/evidence/M05/commands.txt`.
- Decisions: Service Bus uses a Standard candidate because duplicate detection is unavailable in Basic. Container Apps use separate user-assigned identities and scoped RBAC instead of registry passwords or inline secrets. Restricted public endpoints remain the local contract while private networking is deferred. Terraform Registry package resolution was the only external access; no Azure account, subscription, provider configuration, backend, refresh, plan, apply, destroy, networking activation, resource, or cost was used. Every candidate region/SKU/price remains unapproved.
- Blockers: M5 remains open for secretless OIDC, named Azure ownership and scopes, current cost/service validation, a saved subscription-backed plan, explicit apply approval, ephemeral cloud verification, rollback/restore, scoped destroy, and independent teardown reconciliation. The user has explicitly deferred every Terraform execution against Azure. Final M4 external/provider/native-device proof also remains open.
- Next exact task: Build `M05-secretless-oidc-plan-contract-local-004` as static local preparation only: define GitHub environment/branch claims, separate least-privilege plan versus apply/destroy identity contracts, CI permissions, and fail-closed command-policy tests without creating federated credentials, adding live provider configuration, contacting Azure, or running a subscription-backed plan.

### 2026-08-28 — Secretless OIDC plan contract passed locally

- Milestone: M5 Azure dev foundation and continuous delivery in Phase A local preparation; all Azure and GitHub deployment activation remains deferred
- Completed: Added a versioned static contract for the exact repository, issuer/audience, immutable owner/repository-ID subject templates, protected `main` environments, commit-pinned future actions, external identifier variable names, and a prohibition on long-lived Azure credentials. Numeric GitHub IDs must come from the reviewed subject preview during a future approved activation rather than from guessed values.
- Completed: Separated future plan, apply, and destroy authority across three identities and environments. Added exact saved-plan commands, required commit/digest/cost/approval/expiry/target evidence, four accepted invocations, and twenty fail-closed scenarios spanning claims, runner, permissions, approvals, identity separation, command, root, and target scope. Added one inactive three-job example outside `.github/workflows`; current CI remains `contents: read` only and cannot request an OIDC token or deploy.
- Verification: `pnpm azure-oidc:check` passed three distinct identities, four accepted invocations, twenty refusal scenarios, one inactive template, and one active non-deploying workflow. The complete pinned Node 24.19.0/pnpm 11.24.0 repository gate passed all nine workspaces, all existing tests including 111 mobile tests, Terraform mock checks, contracts, and builds. The security scan passed 539 text files, and Gitleaks found no leaks in approximately 15.38 MB.
- Evidence: `config/azure-oidc-plan-gate.v1.json`, `scripts/validate-azure-oidc-plan-gate.mjs`, `infra/terraform/ci/azure-workload-identity.workflow.yml.example`, `infra/terraform/ci/README.md`, `docs/operations/github-azure-oidc-plan-gate.md`, `docs/evidence/M05/summary.md`, and `docs/evidence/M05/commands.txt`.
- Decisions: `id-token: write` is permitted only in the inactive future job template and only at job scope with `contents: read`. Plan authority is read-only except its exact state object/lock; apply consumes only a reviewed saved plan; destroy first creates and then consumes a separately reviewed saved destroy plan. No GitHub environment, Azure/Entra identity, federated credential, Azure action, Azure account, backend, provider-backed plan, apply/destroy, resource, or cost was created or used.
- Blockers: M5 remains open for current region/SKU/cost approval, named Azure ownership and numeric GitHub IDs, actual environments/identities/federated credentials, a saved subscription-backed plan, explicit apply approval, cloud testing, rollback/restore, scoped destroy, and independent live-state reconciliation. The user has explicitly deferred every Azure execution step. Final M4 external/provider/native-device proof also remains open.
- Next exact task: Build `M05-saved-plan-evidence-contract-local-005` as static local preparation only: define a sanitized saved-plan manifest and digest, cost/approval/expiry/target evidence, producer-consumer binding, and synthetic refusal tests. Do not create GitHub or Azure resources, authenticate, initialize remote state, request live prices, run a provider-backed plan, apply, destroy, or activate the inactive workflow.

### 2026-08-28 — Saved-plan evidence contract passed locally

- Milestone: M5 Azure dev foundation and continuous delivery in Phase A local preparation; all Azure and GitHub deployment activation remains deferred
- Completed: Added a closed saved-plan evidence schema with separate synthetic apply and destroy manifests. Each binds harmless fixture bytes by SHA-256 to the repository/ref/commit, disposable Terraform root/state key/resource group, current provider lock, canonical 28-resource inventory, zero retained targets, exact OIDC producer/consumer identity and environment, exact saved-plan command, sanitized change counts, synthetic cost ceiling, lifecycle, and transient deletion.
- Completed: Added a canonical review-payload digest spanning source, artifact, target, plan summary, cost, producer, and expiry. Plan review, cost review, and operation approval must use independent actors; consumers must match the artifact and review digests after approval and before expiration. Actual `.tfplan`, `.tfstate`, and `.terraform` artifacts are independently refused even though Git ignores them.
- Verification: `pnpm saved-plan:check` passed two producer-consumer manifests, forty-three refusal scenarios, all eight OIDC evidence fields, exact provider-lock and target-inventory digests, and zero checked-in plan/state artifacts. The complete pinned Node 24.19.0/pnpm 11.24.0 repository gate passed all nine workspaces and all existing tests including 111 mobile tests. The security scan passed 546 text files, and Gitleaks found no leaks in approximately 15.44 MB.
- Evidence: `config/saved-plan-evidence.v1.json`, `config/fixtures/saved-plan-evidence/`, `scripts/validate-saved-plan-evidence.mjs`, `docs/operations/saved-plan-evidence-gate.md`, `docs/evidence/M05/summary.md`, and `docs/evidence/M05/commands.txt`.
- Decisions: A plan digest proves byte identity, not correctness, safety, cost approval, or mutation authority. Actual Terraform plan binaries may exist only as short-lived protected future workflow artifacts and may never be committed or retained as evidence because they can contain sensitive values. Current dollar amounts, actors, runs, reviews, approvals, commits, and payloads are visibly synthetic and make no live claim.
- Blockers: M5 remains open for current Azure region/SKU/price review, named ownership and monitored cost alerts, actual GitHub environments/OIDC identities/backend, a subscription-backed saved plan, explicit same-day apply approval, cloud testing, rollback/restore, scoped destroy, and independent Terraform/live-Azure reconciliation. The user has explicitly deferred every Azure execution step. Final M4 external/provider/native-device proof also remains open.
- Next exact task: Build `M05-ephemeral-run-ledger-contract-local-006` as static local preparation only: define the apply/test/rollback-or-continue/destroy/reconcile state machine, separate disposable and retained inventories, evidence completeness, failure escalation, and synthetic refusal tests. Do not create identities or cloud resources, authenticate, initialize remote state, request prices, run a provider-backed plan, apply, destroy, or activate CI.

### 2026-08-28 — Ephemeral run ledger passed locally

- Milestone: M5 Azure dev foundation and continuous delivery in Phase A local preparation; all Azure and GitHub deployment activation remains deferred
- Completed: Added a 23-state, 29-transition evidence ledger from reviewed saved-plan readiness through apply, eleven required cloud-test categories, continue or application-revision rollback, reviewed destroy, independent Terraform-state/live-resource reconciliation, and complete or escalated termination. Every event has a contiguous sequence, legal transition, monotonic New York timestamp, synthetic actor/evidence reference, and reason code.
- Completed: Added three SHA-256-bound synthetic fixtures. The continue fixture passes every gate before clean teardown; the rollback fixture fails integration, restores the prior application revision, then tears down cleanly; the orphan fixture exceeds the destroy timeout, opens an attached incident, independently finds one live resource absent from state, and remains escalated rather than falsely complete.
- Completed: Separated the exact 28-resource disposable inventory from six retained control-plane classes. Complete requires successful destroy, zero Terraform-state objects, zero independently observed live resources, no orphans, exact retained inventory, and passing tests or successful rollback. Apply/rollback/destroy failure, timeout, orphan, or inventory mismatch requires an attached open incident.
- Verification: `pnpm run-ledger:check` passed three terminal ledgers, twenty-three states, twenty-nine transitions, eleven required gates, and fifty-nine refusal scenarios. The complete pinned Node 24.19.0/pnpm 11.24.0 repository gate passed all nine workspaces and all existing tests including 111 mobile tests. The security scan passed 552 text files, and Gitleaks found no leaks in approximately 15.52 MB.
- Evidence: `config/ephemeral-run-ledger.v1.json`, `config/fixtures/ephemeral-run-ledger/`, `scripts/validate-ephemeral-run-ledger.mjs`, `docs/operations/ephemeral-run-ledger-gate.md`, `docs/evidence/M05/summary.md`, and `docs/evidence/M05/commands.txt`.
- Decisions: Terraform output alone never proves teardown; the final report must show **Disposable workload: empty** separately from **Retained control plane: expected list** using independent state and live queries. A failed cleanup remains attached and escalated until independently reconciled. The timeout values are planning contracts, not Azure measurements. Every fixture value and outcome is synthetic.
- Blockers: M5 remains open for current Azure region/SKU/price review, named ownership and monitored cost alerts, actual GitHub environments/OIDC identities/backend, a subscription-backed saved plan, explicit same-day apply approval, cloud testing, live rollback/restore, scoped destroy, and independent Terraform/live-Azure reconciliation. The user has explicitly deferred every Azure execution step. Final M4 external/provider/native-device proof also remains open.
- Next exact task: Build `M05-recovery-drill-contract-local-007` as static local preparation only: define PostgreSQL point-in-time restore, Blob recovery/retention, Container Apps revision rollback, migration/data reconciliation, evidence completeness, timeout/failure escalation, and synthetic refusal tests. Do not create identities or resources, authenticate, initialize remote state, request prices, run a provider-backed plan, apply, restore, roll back, destroy, or activate CI.

### 2026-08-28 — Recovery drill contract passed locally

- Milestone: M5 Azure dev foundation and continuous delivery in Phase A local preparation; all Azure and GitHub deployment activation remains deferred
- Completed: Added four canonical SHA-256-bound synthetic drills covering successful PostgreSQL point-in-time restore, privacy-safe Blob version/soft-delete recovery, successful previous Container Apps revision restore, and a PostgreSQL timeout that remains attached to an open escalation. The contract is bound to the current seven-day PostgreSQL PITR safeguard, private versioned Blob storage with seven-to-fourteen-day soft delete, one retained inactive Container Apps revision, immutable image digests, and the current 20-entry forward-only migration manifest.
- Completed: Added draft planning windows for RPO, RTO, and escalation timeout plus seven exact reconciliation categories for migration manifest, row counts, public IDs, status histories, audit records, privacy exclusions, and application readiness. Expired or deleted privacy-limited data cannot return to active use, and traffic remains drained until recovery and reconciliation pass.
- Verification: `pnpm recovery-drill:check` passed four synthetic drills, seven reconciliation checks, and sixty-two expected mutation/refusal cases with zero cloud operations. The complete pinned Node 24.19.0/pnpm 11.24.0 repository gate passed all nine workspaces, all existing tests including 111 mobile tests, the 20-entry migration manifest, contracts, and builds. The security scan passed 559 text files, and Gitleaks found no leaks in approximately 15.59 MB.
- Evidence: `config/recovery-drill.v1.json`, `config/fixtures/recovery-drill/`, `scripts/validate-recovery-drill.mjs`, `docs/operations/recovery-drill-gate.md`, `docs/evidence/M05/summary.md`, and `docs/evidence/M05/commands.txt`.
- Decisions: The RPO/RTO values are internal planning targets, not Azure guarantees. Complete requires an isolated target, immutable reviewed artifacts, source non-mutation, synthetic evidence, in-window recovery, matching reconciliation, and privacy-safe exclusions. Timeout, failure, ambiguity, or mismatch remains escalated with an attached open incident.
- Blockers: M5 remains open for current Azure service/SKU/price review, named ownership and monitored cost alerts, actual GitHub environments/OIDC identities/backend, a subscription-backed saved plan, explicit same-day apply approval, cloud testing, measured live rollback/restore, scoped destroy, and independent Terraform/live-Azure reconciliation. Final M4 external/provider/native-device proof also remains open.
- Next exact task: Stop at this clean local checkpoint. The next M5 step crosses the deferred external boundary: review current Azure services/SKUs/prices and named ownership, then obtain separate approval before any identity, backend, provider-backed plan, deployment, restore, rollback, destroy, live query, networking, or cost-incurring action.

### 2026-08-28 — Dashboard workload module passed locally

- Milestone: M5 Azure dev foundation and continuous delivery in Phase A local preparation; all Azure and GitHub deployment activation remains deferred
- Completed: Added a separately named dashboard Container App and user-assigned identity behind the disabled dev root. It receives ACR pull only, uses an immutable digest, exposes HTTPS-only ingress to the reviewed synthetic CIDR allowlist, receives only non-secret API URL/environment references, and scales from zero to one candidate replica.
- Completed: Rebound the exact mock workload from 28 to 31 resources—three apps, three identities, and nine role assignments—then regenerated the canonical synthetic saved-plan review digests and all three run-ledger digests. Added a dashboard-specific unsafe-scale refusal while preserving zero ordinary changes and the retained/disposable boundary.
- Verification: `pnpm terraform:check` passed two roots, three plan tests, twelve refusals, zero default changes, 31 mock-only creates across 17 types, eleven tags, fourteen safeguards, twelve low-cost defaults, and eight blocked external gates. Saved-plan, run-ledger, and recovery gates passed 43, 59, and 62 refusal scenarios. TFLint and validator ESLint reported zero findings. The complete pinned repository gate passed all nine workspaces, 111 mobile tests, the 20-entry migration manifest, contracts, and builds; the security scan passed 560 text files, and Gitleaks found no leaks in approximately 15.60 MB.
- Evidence: `infra/terraform/modules/workload-container-apps/`, `infra/terraform/environments/dev/`, `config/terraform-foundation.v1.json`, `config/saved-plan-evidence.v1.json`, `config/ephemeral-run-ledger.v1.json`, `docs/evidence/M05/summary.md`, and `docs/evidence/M05/commands.txt`.
- Decisions: The first mock test failed only because Terraform list and tuple equality is type-sensitive; deterministic joined-string assertions replaced it. A diagnostic local `.terraform` cache was correctly refused by the saved-plan gate and moved to Trash before verification. No Azure/GitHub/Entra resource, credential, backend, provider-backed plan, live price request, mutation, networking, customer data, or cost was used; all resource and cost values remain synthetic and unapproved.
- Blockers: M5 remains open for current Azure service/SKU/price review, named ownership and monitored cost alerts, actual environments/OIDC identities/backend, a subscription-backed saved plan, explicit same-day apply approval, cloud testing, measured live rollback/restore, scoped destroy, and independent Terraform/live-Azure reconciliation. Final M4 external/provider/native-device proof also remains open.
- Next exact task: Stop at this local checkpoint. The next M5 step crosses the user-deferred external boundary and requires separate approval before any Azure service/price review, identity, backend, provider-backed plan, deployment, restore, rollback, destroy, live query, networking, or cost-incurring action.

### 2026-08-28 — Container image contract passed locally

- Milestone: M5 Azure dev foundation and continuous delivery in Phase A local preparation; all image-registry, Azure, and GitHub deployment activation remains deferred
- Completed: Added API, dashboard, and worker Dockerfiles that fail closed unless a future build supplies a reviewed digest-pinned Node 24.19.0 base plus full commit, UTC build time, and semantic version. All three use frozen pnpm 11.24.0, restricted source copies, numeric `10001:10001` ownership, OCI provenance labels, exact commands, and no secret build arguments; API/dashboard include local health checks.
- Completed: Added production package allowlists, monorepo-aware Next.js standalone output, and a worker process that remains alive until clean `SIGINT`/`SIGTERM` shutdown. Added a machine contract, synthetic build-input fixture, restricted `.dockerignore`, operations gate, and a source-preserving validator that uses an isolated offline temporary workspace.
- Verification: `pnpm container:check` assembled three production bundles, passed API liveness/build provenance, dashboard render, and worker lifecycle checks, passed thirty-five refusals, retained seven external gates, removed every temporary bundle, and performed zero Docker or registry operations. Validator/application ESLint and full `pnpm verify` passed all nine workspaces, 111 mobile tests, the 20-entry migration manifest, OpenAPI contracts, builds, and existing M4/M5 gates. The security scan passed 569 text files; Gitleaks found no leaks in approximately 15.66 MB.
- Evidence: `.dockerignore`, `apps/api/Dockerfile`, `apps/dashboard/Dockerfile`, `apps/worker/Dockerfile`, `config/container-image-contract.v1.json`, `config/fixtures/container-images/`, `scripts/validate-container-build-inputs.mjs`, `scripts/validate-container-images.mjs`, `docs/operations/container-image-gate.md`, `docs/evidence/M05/summary.md`, and `docs/evidence/M05/commands.txt`.
- Decisions: A production bundle smoke is not an OCI image build. The base image has no default, and the synthetic digest/tag fixture does not claim an image exists. Future base review, Docker build, vulnerability scan, keyless signature/provenance, registry push, resulting digest binding, and Azure deploy remain separate approval gates. The active CI remains non-deploying and performs only local bundle verification.
- Blockers: M5 remains open for current Azure service/SKU/price review, named ownership and monitored cost alerts, real GitHub environments/OIDC identities/backend, a reviewed Node base digest, built/scanned/signed/published images, a subscription-backed saved plan bound to image digests, explicit same-day apply approval, cloud tests, live rollback/restore, scoped destroy, and independent Terraform/live-Azure reconciliation. Final M4 external/provider/native-device proof also remains open.
- Next exact task: Stop at this local checkpoint. The next M5 work requires external review or execution and separate approval before any base-image/registry request, Azure service/price review, identity, backend, provider-backed plan, Docker build/pull, image scan/sign/push, deployment, restore, rollback, destroy, live query, networking, or cost-incurring action.

### 2026-08-28 — Consolidated M5 local preflight passed

- Milestone: M5 Azure dev foundation and continuous delivery in Phase A local preparation; M5 itself remains open and every live image, Azure, and GitHub deployment gate remains deferred
- Completed: Added one fail-closed preflight manifest that reconciles the Terraform, OIDC, saved-plan, run-ledger, recovery, and container-image contracts; nine local preparation areas; fifteen required artifacts; shared 31-resource, expiration, identity-environment, recovery, and image-execution boundaries; active read-only CI; and sixteen separately owned external gates.
- Completed: Added forty-five exercised refusal scenarios for false completion or execution claims, missing/duplicate contracts and evidence, completed or unapproved external gates, active-CI OIDC/write/Azure/Terraform/Docker/registry drift, cross-contract count/time/identity drift, and Terraform plan/state/cache artifacts. The gate is part of `pnpm verify` and does not run Docker, a registry, Azure, a remote backend, or Terraform mutations.
- Verification: `pnpm m5:preflight` passed six contracts, nine local coverage areas, sixteen deferred external gates, and forty-five exercised refusals. The complete pinned Node 24.19.0/pnpm 11.24.0 `pnpm verify` gate passed all existing prerequisite, auth, authorization, OIDC, saved-plan, ledger, recovery, Terraform, workspace lint/type/test/contract/build, and three-bundle container checks. The security scan passed 572 text files, and Gitleaks found no leaks in approximately 15.69 MB.
- Evidence: `config/m5-local-preflight.v1.json`, `scripts/validate-m5-local-preflight.mjs`, `docs/operations/m5-local-preflight-gate.md`, `docs/evidence/M05/summary.md`, and `docs/evidence/M05/commands.txt`.
- Decisions: This is a readiness audit, not cloud evidence and not the M5 gate. Each external gate requires its own owner, evidence, and explicit approval; approval to review current prices or services does not authorize identity creation, backend initialization, planning, applying, recovery, destruction, or any later mutation.
- Blockers: The first unchecked M5 item requires external review and cloud execution currently prohibited by the user boundary. Final M4 Entra/provider/native-device proof also remains open, and the plan forbids advancing to M6 before M5 passes.
- Next exact task: Obtain explicit approval only for the current Azure service/SKU/price/region and named-owner review. Keep identity, backend, provider-backed plan, Docker build/pull/scan/sign/push, deployment, restore, rollback, destroy, live query, networking, customer data, and cost separately blocked until their own gates are approved.

### 2026-08-30 — Public Azure service, region, price, and ownership review passed

- Milestone: M5 Azure dev foundation and continuous delivery at the pre-plan approval boundary; M5 remains open and no Azure subscription or resource has been touched
- Completed: Queried Microsoft's unauthenticated public Retail Prices API for East US 2 and reviewed current Microsoft documentation for PostgreSQL, Container Apps, Service Bus, Container Registry, Storage, Key Vault, Log Analytics, and workspace-based Application Insights. Retained PostgreSQL 16 B1ms/32 GiB/seven-day LRS backup, three zero-to-one Consumption Container Apps, Service Bus Standard, ACR Basic, Hot LRS Blob storage, Standard Key Vault, and capped 30-day telemetry.
- Completed: Assigned Blake Tindol as the accountable human platform, cost, alert-response, technical-on-call, and independent plan-review owner. Codex is the automated plan producer accountable to Blake. The real monitored alert destination remains an external value and no contact detail or credential was committed.
- Completed: Calculated a deliberately conservative `$3.02` eight-hour public-retail estimate, including full active Container Apps runtime, one million requests, charged Log Analytics despite the public free tier, full-day ACR, storage transaction blocks, and PostgreSQL/Service Bus time. Proposed a `$1.98` uncertainty reserve, `$5.00` per-run ceiling, `$25.00` monthly budget, and 50%/80%/100% actual and forecast alerts. The proposal is not yet approved or configured.
- Verification: `pnpm azure-cost:check` passed seven reviewed service groups, the exact cost arithmetic, Terraform default coherence, official-source restrictions, the seven-day review window, accountable ownership, pending approval, and twenty-six exercised refusals. The repository's pinned Node 24.19.0/pnpm 11.24.0 verification, security scan, Gitleaks, and whitespace results are recorded in `docs/evidence/M05/commands.txt`.
- Evidence: `config/azure-public-cost-review.v1.json`, `scripts/validate-azure-public-cost-review.mjs`, `docs/operations/azure-public-service-cost-review.md`, `docs/external-prerequisites.md`, `docs/operations/ephemeral-azure-development.md`, `docs/evidence/M05/summary.md`, and `docs/evidence/M05/commands.txt`.
- Decisions: East US 2 is selected for the first ephemeral development plan from public evidence, but subscription registration, offer, quota, policy, and moment-of-plan availability remain unverified. Requery public prices after 2026-09-06 or immediately before provider planning. Public retail data is an estimate, not a quote, budget approval, provider plan, or Azure execution authorization.
- Blockers: The next action requires Azure authentication and read access to the intended subscription plus exact scope, monitored alert, budget, identity, backend, and artifact references. Apply, deployment, recovery, destroy, CI activation, and all cost-incurring mutations remain separately blocked.
- Next exact task: Stop and obtain explicit user approval before any Azure login, account/subscription read, provider/backend initialization, subscription-backed Terraform plan, identity or registry operation, networking, resource creation, or cost. If approved, generate and return only a saved provider-backed plan and current cost summary; do not apply it.

### 2026-08-31 — iOS readiness and Azure architecture review prepared locally

- Milestone: M5 Azure dev foundation and continuous delivery at the pre-plan approval boundary; M5 remains open, final M4 external/native proof remains deferred, and no Azure access was used
- Completed: Reconciled the current iOS state against the live checklist, app configuration, native M1/M2 evidence, M3/M4 local contracts, and later release gates. Recorded the truthful distinction between the proven Expo Go Simulator prototype and the still-missing signed physical-device, real Entra, Azure-connected, TestFlight, and production gates.
- Completed: Added a senior-principal Azure development reference architecture with official Microsoft Azure and Entra SVG icons, numbered runtime/identity/RBAC/control flows, retained-versus-disposable ownership, security boundaries, exact M5 service candidates, three user-assigned identities, explicit public-network Phase B posture, deferred private-network Phase C posture, and current not-deployed/activation-disabled labels.
- Verification: Current pinned Node 24.19.0/pnpm 11.24.0 mobile unit tests passed 111/111; mobile TypeScript and Expo web export passed; the Maestro source contract passed two flows and 34 test-ID references. The SVG passed `xmllint`, rendered to a 5600 × 4200 exact-label PNG, and was visually inspected. The requested imagegen skill produced a separate 1536 × 1024 presentation render; visual review caught and corrected one role-label drift before the final render was retained. The actual iOS Simulator was not rerun in this session; retained native and Maestro evidence remains the truthful device proof.
- Evidence: `docs/architecture/ios-readiness-and-azure-review.md`, `docs/architecture/local-missions-azure-reference-architecture-imagegen.png`, `docs/architecture/local-missions-azure-reference-architecture.svg`, `docs/architecture/local-missions-azure-reference-architecture.png`, `docs/architecture/azure-icons/`, `docs/evidence/M01/summary.md`, `docs/evidence/M02/summary.md`, `docs/evidence/M02/maestro-runs.md`, `docs/evidence/M02/voiceover-device-gate.md`, and `docs/evidence/M04/summary.md`.
- Decisions: The diagram is the current M5 development topology, not a deployment claim or a final production edge. Stripe, notification delivery, App Store/EAS, Reach providers, and later production integrations remain outside this M5 resource view. Imagegen supplies the polished presentation render; the deterministic SVG remains the authoritative exact-label, trust-boundary, and official-icon control copy.
- Blockers: A real Entra/system-browser/deep-link/physical-device M4 gate remains open. The next M5 action still requires explicit Azure read-only/plan-only approval plus exact subscription, backend, identities, monitored alert destination, budget, and plan artifact references. Apply, deployment, registry, cost, cloud testing, recovery, destroy, and networking mutations remain separately blocked.
- Next exact task: Review and approve or revise the architecture and test sequence. Do not authenticate to Azure or generate a provider-backed plan until the user explicitly says: `Azure read-only/plan-only access. No apply or deployment.`

### 2026-08-31 — Local Missions naming and Azure provider-scope plan passed

- Milestone: M5 Azure dev foundation and continuous delivery at the retained-bootstrap/full-create-plan approval boundary; M5 remains open and no Azure resource has been created or changed.
- Completed: Added a Local Missions-only naming contract using readable `local-missions` names, compact `lm` names where Azure requires them, and a never-reused `e2rYYMMDDNN` stamp for each disposable East US 2 run. Existing `rg-pp-*` resources, state, identities, and control objects are explicitly forbidden targets. Shared-subscription placement requires separate co-tenancy approval; a dedicated Local Missions subscription remains preferred before staging.
- Completed: Added four machine-validated cost profiles: zero-resource `plan-only`, `$2`/2-hour smoke, `$3`/4-hour integration, and `$5`/8-hour full validation, all bounded by same-day 11:00 PM New York teardown and the proposed `$25` monthly budget. Added sixteen exact workload tags including application code, region, stamp, cost profile, and run ceiling.
- Completed: Added a hardened AzureRM 5.0.1 provider with automatic resource-provider registration disabled, Entra-based Storage access, safe Key Vault soft-delete behavior, and guarded subscription/tenant equality checks whose UUID inputs remain process-only and sensitive. The ordinary plan stopped at the unbootstrapped Local Missions backend rather than reusing the other workload's state. A dedicated real-provider Terraform `command = plan` test then passed against ephemeral test state with zero additions, changes, or destroys.
- Verification: `pnpm azure-naming:check` passed 15 resource templates, four tiers, and 18 refusals. `pnpm terraform:check` passed two roots, three local plan tests, 12 refusals, zero default resources, and the 31-resource mock-only shape. Live read-only checks found no Local Missions resource group and found the proposed workload Storage, ACR, Key Vault, and retained state-storage names available. The provider-scope plan passed 1/1 with no saved plan, apply, backend initialization, provider registration, resource mutation, or retained account identifier.
- Evidence: `config/azure-naming.v1.json`, `config/azure-provider-scope-plan.v1.json`, `scripts/validate-azure-naming.mjs`, `scripts/validate-azure-provider-scope-plan.mjs`, `docs/operations/azure-naming-and-cost-tiers.md`, `docs/evidence/M05/azure-provider-scope-plan.md`, `infra/terraform/environments/dev/provider.tf`, and `infra/terraform/environments/dev/provider-tests/provider-scope.tftest.hcl`.
- Decisions: Name availability is a point-in-time read, not a reservation. Daily deletion applies only to the exact stamped disposable workload; retained state, budgets, identities, cleanup controls, and every other project's Azure resources remain outside that destroy target. Public retail cost figures remain estimates rather than quotes.
- Blockers: Full provider-backed create planning still needs an approved subscription placement, a separate retained Local Missions backend/control plane, monitored budget destination, plan/apply/destroy identities, PostgreSQL administrator group, narrow real CIDRs, and built/scanned/signed/published immutable images. Apply remains separately prohibited.
- Next exact task: Stop for approval. If approved, bootstrap only the retained Local Missions state/control boundary and identities under an explicit mutation/cost scope, then generate the saved 31-resource create plan for independent review. Do not apply the workload until a second explicit approval selects a cost tier and same-day teardown window.

### 2026-08-31 — Retained foundation and two-phase workload plan passed locally

- Milestone: M5 Azure dev foundation and continuous delivery at the first Azure-mutation approval boundary; M5 remains open and no retained or disposable Azure resource has been created or changed.
- Completed: Added a separate three-resource bootstrap root for the Local Missions state resource group, Entra-only/default-deny/infrastructure-encrypted/versioned Storage account, and private state container. All three objects use `prevent_destroy`; retained state has a conservative `$1/month` ceiling and must migrate immediately from the one-time local bootstrap backend after any approved apply.
- Completed: Replaced the empty control-plane root with a mock-validated nine-resource retained shape: one control group, separate plan/apply/destroy managed identities, three exact immutable GitHub environment federated credentials, one monitored Action Group, and one `$25` budget filtered to `application=local-missions` with 50%/80%/100% actual and forecast notifications. Account UUIDs and the monitored receiver remain sensitive process-only inputs. Workflow RBAC remains intentionally unimplemented until dedicated-versus-shared subscription placement establishes safe scopes.
- Completed: Corrected the non-applyable single-pass workload design. The first workload phase now creates 28 core resources—including the disposable ACR, Container Apps environment, application identities, and scoped RBAC—but zero Container App resources. Only after the API/dashboard/worker images are built, scanned, signed, pushed, and revalidated by digest may `application_activation_enabled` add exactly those three apps and reach the 31-resource final workload.
- Verification: `pnpm terraform:check` passes three roots, seven plan tests, twenty refusal tests, zero default changes, the three-resource mock bootstrap, nine-resource mock control plane, 28-resource mock core, and 31-resource mock activated workload across twenty Azure resource types. Source validation inspects retained destroy protection, state network/auth/recovery controls, separate immutable GitHub federation, six budget alerts, Local Missions budget filtering, no client secrets, and second-phase app counts. Official GitHub/Microsoft documentation confirms immutable subjects append owner/repository IDs and Entra requires exact case-sensitive subject matching.
- Evidence: `infra/terraform/bootstrap/`, `infra/terraform/control-plane/`, `infra/terraform/environments/dev/`, `infra/terraform/modules/workload-container-apps/`, `config/terraform-foundation.v1.json`, `config/azure-plan-readiness.v1.json`, `scripts/validate-terraform-foundation.mjs`, `scripts/validate-azure-plan-readiness.mjs`, and `docs/operations/azure-retained-foundation-and-two-phase-plan.md`.
- Decisions: The retained backend cannot create itself while already using itself, and Container Apps cannot consume images from a disposable ACR before that ACR exists. Bootstrap, control plane, core, image publication, application activation, and destroy are therefore separately reviewed operations. Daily deletion covers only the exact stamped workload; retained Local Missions control objects and every `rg-pp-*` object remain outside scope.
- Blockers: The owner must decide dedicated versus shared subscription placement and approve the privilege model. Retained cost/bootstrap, monitored destination, immutable GitHub subject preview, RBAC assignments, PostgreSQL administrator group, trusted operator network rule, base image digest, and every mutation/apply remain unapproved. No saved create plan exists.
- Next exact task: Stop before Azure mutation. With explicit approval, generate only the three-resource retained bootstrap saved plan first. Do not apply it, create control identities/budget, or generate the workload-core plan under that approval unless those later operations are separately authorized.

### 2026-08-31 — Shared-subscription landing zone and constrained workflow RBAC passed locally

- Milestone: M5 Azure dev foundation and continuous delivery at the owner subscription-placement decision; M5 remains open and no Azure resource was created, changed, or deleted.
- Completed: Corrected the remaining shared-subscription privilege risk. The control plane now owns an empty retained `rg-local-missions-dev-eus2-001` landing zone, so plan/apply/destroy workflows never require subscription-scope workload permissions or daily resource-group recreation. Dedicated Local Missions subscription remains preferred; explicitly approved shared nonproduction co-tenancy is now technically bounded to the same app-only group.
- Completed: Added five landing-zone-scoped workflow assignments: Reader for plan, Contributor for apply/destroy, and Role Based Access Control Administrator for apply/destroy. Both privileged assignments use Azure condition version `2.0` and limit assignment create/delete to five reviewed application data roles and `ServicePrincipal` recipients. Owner, subscription Contributor, User Access Administrator, unconstrained role delegation, and other-project targets remain forbidden.
- Completed: Moved resource-group ownership out of the disposable root. The workload now validates the exact landing-zone name and retained-boundary tags, creates 27 stamped core resources, and later adds exactly three Container Apps for a 30-resource activated workload. Daily teardown empties workload state and independently proves zero exact-stamp resources while retaining the empty landing zone, control plane, and remote state.
- Verification: `pnpm azure-placement:check` passed two owner-gated placement options, five workflow assignments, five delegated roles, and twenty refusal scenarios. `pnpm azure-plan-readiness:check` passed three roots, ten ordered stages, fourteen gates, and twenty-eight refusals. `pnpm terraform:check` passed three roots, seven plan tests, twenty-two Terraform refusals, zero default changes, three bootstrap creates, fifteen control-plane creates, a 27-resource core, and a 30-resource activated workload across twenty Azure resource types.
- Evidence: `config/azure-subscription-placement.v1.json`, `scripts/validate-azure-subscription-placement.mjs`, `infra/terraform/control-plane/`, `infra/terraform/environments/dev/`, `docs/operations/azure-subscription-placement-and-rbac.md`, `config/terraform-foundation.v1.json`, and `config/azure-plan-readiness.v1.json`.
- Decisions: Resource groups are retained security/lifecycle boundaries; metered resources inside the Local Missions landing zone are disposable. The workload root cannot delete the group. Constrained delegation is required because the workload creates nine narrow application data-role assignments; unrestricted access administration is prohibited.
- Blockers: The owner must still select dedicated versus shared placement. Bootstrap cost/apply, a monitored alert destination, exact immutable GitHub subjects, PostgreSQL administrator group, trusted operator network, reviewed base image digest, plan/apply/destroy approvals, registry activity, and every Azure mutation remain unapproved.
- Next exact task: Stop before Azure-backed create planning until the owner selects placement. After explicit plan-only approval, generate only the three-resource retained-state bootstrap saved plan; do not apply it or plan later phases under that approval.

### 2026-08-31 — Dedicated bootstrap scope gate refused the shared subscription

- Milestone: M5 Azure dev foundation and continuous delivery at the dedicated-subscription bootstrap-plan boundary; no apply, deployment, registration, backend initialization, or resource mutation occurred.
- Completed: Replaced built-in Contributor workflow assignments with Local Missions-only deployer/destroyer custom roles. Apply cannot delete resources; destroy can delete stamped workload resources but cannot delete the retained landing-zone group. The retained control-plane mock target is now 17 resources.
- Completed: Recorded the owner's dedicated-subscription selection and bootstrap-plan-only authorization. Read-only Azure inventory found only one enabled visible subscription with four non-Local-Missions resource groups, so it was rejected rather than silently treated as dedicated. Added exact provider-scope, dedicated-isolation, retained-cost, and trusted-network bootstrap gates.
- Verification: Full `pnpm verify` passed formatting, all M4/M5 machine gates, nine-workspace lint/type/test/build/contracts, 111 mobile tests, and the three-bundle container contract. `pnpm terraform:check` passed three roots, seven plan tests, twenty-four refusal tests, zero default changes, three mock bootstrap changes, seventeen mock control-plane changes, and the 27/30-resource workload phases. `pnpm azure-placement:check` passed twenty-two refusals. The high-confidence scan passed 633 text files and Gitleaks found no leaks in approximately 17.62 MB. No saved plan was generated because the selected dedicated target is not visible.
- Evidence: `infra/terraform/bootstrap/`, `infra/terraform/control-plane/`, `config/terraform-foundation.v1.json`, `config/azure-subscription-placement.v1.json`, `config/azure-plan-readiness.v1.json`, and `docs/operations/azure-subscription-placement-and-rbac.md`.
- Decisions: The existing Post Proof development subscription cannot satisfy the selected dedicated Local Missions boundary. Subscription and tenant identifiers remain process-only and are not retained in source or evidence.
- Blockers: A dedicated Local Missions subscription must be created or made visible to the signed-in account. Retained-state cost acceptance and a real narrow trusted operator IPv4 rule are also required before an applyable bootstrap saved plan can be generated. Apply remains separately prohibited.
- Next exact task: Re-run read-only account inventory after the dedicated subscription becomes visible, verify zero other workloads and exact tenant/subscription scope, then generate only the three-resource bootstrap saved plan for independent review.

### 2026-08-31 — Dedicated Local Missions subscription created and isolated

- Milestone: M5 Azure dev foundation and continuous delivery at the retained-state bootstrap-plan boundary; the dedicated subscription now exists, but no Terraform plan, apply, deployment, provider registration, backend initialization, or Azure resource creation occurred.
- Completed: Created `Local Missions Development` under the sole active Microsoft Customer Agreement billing account, billing profile, and invoice section. Only the standard Microsoft Azure Plan was available, so no unavailable Dev/Test offer was inferred.
- Verification: Azure reports the new subscription as enabled in the current tenant with zero resource groups, zero resources, and one subscription-level Owner assignment for the signed-in operator. The existing Post Proof subscription remains the CLI default to avoid silently redirecting other-project commands.
- Decisions: Subscription and tenant identifiers remain process-only and are not retained in source or evidence. The Local Missions subscription is the dedicated isolation boundary and contains no other workload.
- Blockers: Retained-state cost acceptance and a real narrow trusted operator IPv4/CIDR are still required before generating the three-resource bootstrap saved plan. Apply remains separately prohibited.
- Next exact task: After those two inputs are approved, target the dedicated subscription by exact process-only ID, revalidate the provider scope, generate only the three-resource bootstrap saved plan, independently inspect it, and stop before apply.

### 2026-08-31 — Dedicated retained-state bootstrap saved plan reviewed

- Milestone: M5 Azure dev foundation and continuous delivery at the bootstrap apply-approval boundary; apply, deployment, provider registration, remote-backend initialization, and later-phase planning remain prohibited.
- Completed: Corrected the bootstrap cost gate so this three-resource phase requires the owner-approved `$1/month` retained-state ceiling rather than falsely implying approval of the later `$25` workload budget. Detected the current Mac public IPv4 for one exact process-only storage firewall rule without retaining the value in source or sanitized evidence.
- Completed: Generated `local-missions-bootstrap.tfplan` with Terraform 1.15.7 and AzureRM 5.0.1 against the exact dedicated subscription/tenant. Separate JSON inspection proved exactly three create actions: one retained resource group, one hardened StorageV2 LRS account, and one private state container; there are zero updates, deletes, replacements, control-plane resources, or workload resources.
- Security review: The plan preserves default-deny networking with one exact IPv4 rule, Entra-default storage authentication, disabled Shared Key/local users/anonymous access, TLS 1.2, infrastructure encryption, versioning/change feed, 30-day Blob/container recovery, and `prevent_destroy`. Secret-shaped storage connection-string fields are unknown/sensitive metadata only; zero known secret-shaped string values exist.
- Verification: The binary is mode `0600`, 11,352 bytes, and SHA-256 `5ae5916eb45d21fdf6a2c22daf24056ee7a98bc4307394e631a0b1518bb376ca`. It and the sensitive raw JSON remain outside the repository. Live Azure inventory after planning remains zero resource groups and zero resources; `Microsoft.Storage` remains `NotRegistered`, proving the plan did not register it.
- Evidence: `config/azure-plan-readiness.v1.json` and `docs/evidence/M05/azure-retained-state-bootstrap-plan.md`. Subscription, tenant, and IPv4 values remain process-only.
- Blockers: The saved plan expires at `2026-09-01T00:21:40+00:00`. Apply is not authorized. Remote-state migration, control-plane planning, the `$25` workload budget, identities/RBAC, registry/images, workload plans, cloud tests, and destroy all remain separately gated.
- Next exact task: Stop. If the owner later authorizes bootstrap apply, first revalidate the plan digest, expiry, source/lock digests, dedicated subscription, current operator IP, and zero-resource inventory; generate a replacement plan instead of consuming this one after expiry or drift.

### 2026-09-01 — Retained-state bootstrap applied and remote state verified

- Milestone: M5.3 complete; M5.4 retained control and landing-zone preparation is now the active gate.
- Completed: Rejected the expired bootstrap artifact, revalidated the dedicated target, owner authority, source/provider lock, current process-only IPv4, globally available Storage name, and zero-resource starting inventory, then generated and independently reviewed a replacement saved plan containing exactly three creates and no other actions.
- Applied: Explicitly registered only `Microsoft.Storage` and applied the exact replacement plan. Azure now contains one retained state resource group, one hardened Standard LRS Storage account, and one private Blob container; there are no control-plane or disposable application resources.
- Security: Verified default-deny networking with one operator IPv4, no bypass, Entra-default authentication, disabled Shared Key/local users/anonymous access, HTTPS/TLS 1.2, infrastructure encryption, versioning/change feed, 30-day recovery, private container access, required tags, and Terraform destroy protection.
- State: Added a temporary operator `Storage Blob Data Contributor` assignment at the exact state-container scope, migrated bootstrap state to `local-missions/bootstrap.tfstate`, and independently proved remote state, locking, version history, clean initialization, and no drift. The consumed plan, refresh plan, one-time local state, and temporary review script were removed.
- Correction: Because Blob data-plane access is separate from management-plane Owner access, the retained control-plane target increased from 17 to 20 resources by adding three private-container assignments for the future plan/apply/destroy identities.
- Evidence: `docs/evidence/M05/azure-retained-state-bootstrap-plan.md`, `config/azure-plan-readiness.v1.json`, `infra/terraform/bootstrap/`, and `infra/terraform/control-plane/`.
- Incident: A malformed first JSON-review pipeline emitted raw Terraform JSON into transient tool output, including process-only account identifiers and the operator IP. No credential, token, access key, connection string, or state payload was committed or retained; the review was rerun through a sanitizer and the incident remains documented.
- Next exact task: Supply and verify the monitored cost-alert destination, then resolve the remaining M5.4 inputs and generate an exact 20-resource saved plan for independent review. Do not apply that control plane without separate approval.

### 2026-09-01 — Retained control-plane saved plan reviewed

- Milestone: M5.4 at the exact control-plane apply-approval gate; no control-plane or workload apply occurred.
- Completed: Configured three main-only GitHub environments with required human review and no administrator bypass, enabled immutable OIDC subjects, supplied the alert destination process-only, created the PostgreSQL administrator security group, and revised the alert budget to `$100`.
- Plan: Generated and independently reviewed the exact 20-resource retained control-plane saved plan. It contains 20 creates, zero changes/deletes/replacements, and zero paid workload resources. The artifact remains outside the repository and expires after eight hours.
- Cost: The first future workload run is the `$2` two-hour smoke tier. `$5`/eight hours is a fallback only; the `$100` budget is an alert threshold rather than a spend target or hard cap.
- Evidence: `docs/evidence/M05/azure-control-plane-saved-plan.md`, `config/azure-plan-readiness.v1.json`, `config/azure-oidc-plan-gate.v1.json`, and the three Terraform roots.
- Next exact task: Obtain separate approval bound to the exact control-plane plan SHA-256 and expiry. Do not apply an expired or drifted plan, and do not treat control-plane approval as workload approval.

### 2026-09-01 — Retained control plane applied and verified

- Milestone: M5.4 exact control-plane apply complete; the disposable workload remains undeployed.
- Applied: Revalidated the approved saved-plan hash, mode, expiry, source/provider lock, dedicated target inventory, and empty control state, then applied only that artifact. Terraform reported 20 added, zero changed, and zero destroyed.
- Verification: Proved 20 managed Terraform resources plus one data reference, three exact retained groups, an empty workload landing zone, three managed identities and immutable federated credentials, two custom roles, five workflow assignments, three workflow state-container assignments, one Action Group, and one `$100` budget with six alerts. A normal provider-backed plan reports zero changes.
- Correction: Azure normalized the budget period to September 1, 2026 through September 1, 2027. `prevent_destroy` safely refused the stale August source dates; the local defaults were corrected without Azure mutation before the zero-change plan passed.
- Boundary: The temporary operator state-container role remains until a separately approved GitHub OIDC proof and recovery-path review pass. No workload plan/apply, image/registry activity, Container App, Stripe action, customer data, or iOS deployment occurred.
- Evidence: `docs/evidence/M05/azure-control-plane-saved-plan.md`, `config/azure-plan-readiness.v1.json`, and the live remote control-plane state.
- Next exact task: Approve and run a no-apply GitHub OIDC remote-state/access-policy proof before removing temporary operator access or generating the 27-resource workload-core plan.

### 2026-09-01 — GitHub OIDC and ARM permission proof passed without mutation

- Milestone: M5.4 live workflow federation and authorization proof complete; end-to-end remote-state network access and the disposable workload remain unproven.
- Approval: The owner visibly approved the three protected GitHub environments only for the no-apply proof on commit `73ef5bd5e3251a95347aeb7449d4365745e4c5c4`, explicitly excluding Terraform, deployment, provider registration, and Azure resource mutation.
- Verification: GitHub Actions run `33513053687` completed successfully. Plan, apply, and destroy identities independently exchanged immutable-subject OIDC tokens, acquired management and Storage audience tokens, read the exact retained landing zone, failed closed against the retained control group, proved their operation-specific ARM permissions and container-scoped Blob data actions, and logged out.
- Security: Each real Blob read remained blocked by the unchanged default-deny Storage firewall. This is proof that the network boundary stayed closed, not proof of end-to-end Terraform backend connectivity. Sanitized live inventory still showed the three retained Local Missions groups, an empty workload landing zone, four tagged control/state resources, and four state-container data-role assignments including the temporary operator recovery assignment.
- Evidence: `docs/evidence/M05/github-oidc-proof.md`, the protected-environment review recorded by GitHub, and GitHub Actions run `33513053687`.
- Next exact task: Design and independently review a temporary least-privilege state-network access method and operator recovery path. Do not mutate the Storage firewall/RBAC, remove the operator assignment, or generate the 27-resource workload plan under this completed proof approval.

### 2026-09-01 — GitHub workflow state-network design passed locally

- Milestone: M5.4 at a founder GitHub ownership/billing decision; no organization, paid plan, repository transfer, Azure network, firewall, federation, RBAC, runner, or workload change occurred.
- Finding: The current repository is public under a personal account. GitHub recommends against allowlisting standard-runner ranges and against self-hosted runners on public repositories. Azure Storage public IP rules are not reliable for same-region Azure service traffic, so a dynamic runner-IP exception is not an acceptable automation boundary.
- Proposal: Create the app-only `local-missions-hq` organization if still available, purchase one GitHub Team seat, keep the transferred repository public, and attach a maximum-concurrency-one two-core Linux larger runner to a dedicated East US 2 VNet subnet. Permit the state account only through that subnet's `Microsoft.Storage` service endpoint while retaining default deny, Entra-only access, disabled Shared Key/anonymous access, no trusted-services bypass, and separate protected environments.
- Cost: Current public list estimates are `$4/user/month` for Team and `$0.006/minute` for the two-core Linux runner (`$0.12`/20 minutes or `$0.36`/hour), with no idle runner charge. Azure lists the VNet base as free, but every associated meter remains subject to the exact saved plan and action-time billing review.
- Evidence: `config/github-state-network-gate.v1.json`, `scripts/validate-github-state-network-gate.mjs`, `docs/operations/github-state-network-gate.md`, and proposed `docs/decisions/ADR-060.md`.
- Next exact task: Obtain the founder's explicit approval or rejection of the organization candidate, Team recurring cost, public repository transfer, and first-run runner ceiling. Do not create the organization, purchase Team, transfer the repository, register `GitHub.Network`, or generate/apply an Azure network plan without that decision.

### 2026-09-01 — GitHub Free organization created and federation-only saved plan reviewed

- Checkpoint: `M05-free-org-federation-saved-plan-reviewed-027`.
- GitHub result: The owner rejected paid Team for now. Created `local-missions-hq` on GitHub Free under the personal `stratiosai` account with one active owner, zero repositories, no invitations, and no payment method. No GitHub charge, paid runner, or repository transfer occurred.
- Operating decision: Keep verification and no-apply OIDC/ARM proof on standard hosted runners, keep provider-backed Terraform on the reviewed local operator path, preserve Storage default deny, and defer the paid larger-runner/Azure VNet option to M14.
- Saved plan: Previewed the three future immutable organization/repository environment subjects process-only and generated one real retained-control-plane saved plan outside the repository. Independent review proved three in-place federated-credential subject updates only, zero creates/deletes/replacements, zero network/RBAC/budget/workload changes, mode `0600`, source/provider-lock binding, and SHA-256 `5fbc63430b4778a3e18039109bbe66c065663621fd0025cbd51cffc71a0d3903`.
- Safety: No Terraform apply, Azure mutation, provider registration, firewall/RBAC change, workload plan, repository transfer, Stripe action, or iOS deployment occurred. Private intermediate Terraform logs and JSON were moved to Trash; only the mode-`0600` saved plan remains outside the repository.
- Next exact task: Obtain separate approval bound to the exact federation plan digest and a coordinated still-public repository transfer window. Revalidate every digest and inventory before apply; do not treat that approval as workload or paid-GitHub authorization.

### 2026-09-01 — Public repository transfer completed; post-transfer OIDC correction plan reviewed

- Checkpoint: `M05-post-transfer-oidc-correction-saved-plan-reviewed-029`.
- Azure/Terraform result: Revalidated the approved first plan, dedicated subscription, exact current-IP Storage rule, Entra state access, retained inventory, live old subjects, and GitHub IDs. Applied only that saved plan: `0` added, `3` changed, `0` destroyed. Independent provider-backed reconciliation then reported no changes.
- GitHub result: Transferred the still-public repository to `local-missions-hq` with stable repository ID and redirect. The three protected environments, main-only rules, distinct identifier variables, and zero secrets survived. GitHub reset repository OIDC customization, Actions policy, and branch protection during transfer; restored immutable claim keys, a selected exact-action policy, and solo-founder `main` protection without payment or a paid feature.
- Failed-safe proof: Organization-owned run `33519420112` failed during Azure login before ARM or Blob access. GitHub emitted `repository_owner_id:{id}:repository_id:{id}:environment:{name}`, while the first plan had encoded a name-decorated `repo:...@...` subject. No Terraform command, Azure permission, network/RBAC/workload mutation, or state access occurred.
- Correction plan: Updated the Terraform and OIDC contracts to the observed claim-key format. Generated and independently reviewed mode-`0600` saved plan SHA-256 `de06a09c687092fce1af5476b9ff37fa82d41039c13130e7f51f6395a55f923c`: exactly three in-place federated-credential subject corrections, zero creates/deletes/replacements, and zero network/RBAC/budget/workload changes. It is not applied.
- Next exact task: Obtain separate approval bound to the correction plan digest. If approved, revalidate and apply only the three corrections, then immediately rerun the protected no-Terraform OIDC/ARM proof and require the expected default-deny Blob refusal.

### 2026-09-01 — Corrected post-transfer OIDC/ARM proof passed

- Checkpoint: `M05-post-transfer-oidc-proof-passed-030`.
- Terraform result: Revalidated exact correction plan SHA-256 `de06a09c687092fce1af5476b9ff37fa82d41039c13130e7f51f6395a55f923c`, its source/provider lock, dedicated subscription, current-IP Storage rule, retained inventory, and three live mismatched subjects. Applied only the reviewed artifact: `0` added, `3` changed, `0` destroyed. A normal provider-backed plan returned exit `0` and no changes; the consumed plan was moved recoverably to Trash.
- GitHub proof: Visually approved the three protected jobs only for a no-Terraform OIDC/ARM proof. Run `33521970773` on commit `76803cde194dfc8965bf6114690f47a0dabcdf79` passed plan, apply, and destroy identity token exchange, exact scope/effective permission checks, retained-control denial, expected default-deny Blob refusal, and logout.
