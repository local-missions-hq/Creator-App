...
Current checkpoint: **M05-read-only-recovery-preflight-039**

Next exact task: **After separate approval, generate and independently review the private, saved bootstrap-recovery plan; it may create only the hardened state Storage account and private container, then stop before apply**
...
| Azure | The dedicated `Local Missions Development` subscription has zero disposable workload and zero standing metered compute, database, registry, queue, telemetry, Key Vault, or Storage resources. On the owner's cost-stop instruction, the sole metered retained-state Storage account was deleted after both current Terraform states were copied to a private local recovery directory and validated. Three managed identities, one email-only action group, the budget, custom roles/RBAC, provider registrations, and three empty retained resource groups remain; remote Terraform state is intentionally offline. | Restore the retained state backend from the private backups, reconcile both retained roots, then separately approve the 27-resource workload plan. No Terraform plan/apply may run while the backend is offline. |
...
- [x] Approve exact federation plan SHA-256 `5fbc63430b4778a3e18039109bbe66c065663621fd0025cbd51cffc71a0d3903` and the coordinated transfer window. The approval excluded workload planning and paid GitHub features.
- [x] Revalidate the plan/source/provider-lock digests, dedicated subscription, current IP/state access, unchanged retained inventory, and current ownership; apply exactly three in-place subject updates with zero creates/deletes, then independently prove a normal provider-backed plan has no changes.
...
- [x] Complete the separately approved read-only recovery preflight: uniquely resolve the dedicated subscription without trusting the CLI default; verify three retained groups, four retained management resources, zero Storage and workload resources, budget/RBAC/provider/policy boundaries, two matching public-IPv4 sources, and both private state backups. No Terraform, plan, Azure mutation, or state upload ran.
