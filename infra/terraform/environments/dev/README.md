...
- zero-minimum/one-maximum API and worker replicas, bounded database storage/backup, Basic registry, Standard Service Bus for duplicate-detection support, and short telemetry retention;
...
- separate API/worker managed identities with scoped RBAC, immutable digest image references, RBAC-only Key Vault with no Terraform-managed secret values, and no registry password references;
